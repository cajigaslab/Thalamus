...
char	    AItrigName[256],AIline[256],AOtrigName[256],AOline[256],AOSampClkName[256],DOline[256];
int32       error=0;
char        errBuff[2048]={'\0'};
                  ...
                  std::function<void(std::vector<Channel>*, size_t)> publish, nlohmann::json config)
{
    ...
    for (size_t i = 0; i < num_devices; i++) {
        ...
        if (i == 0) { // only set callbacks on primary device and get AI trigger for other devices
            DAQmxErrChk (DAQmxRegisterEveryNSamplesEvent(devices[i].aiHandle,
            ...
            DAQmxErrChk (GetTerminalNameWithDevPrefix(devices[i].aiHandle,"RTSI0",AIline));
            DAQmxErrChk (DAQmxConnectTerms(AItrigName,AIline,DAQmx_Val_DoNotInvertPolarity));  // connect AI start trigger to RTSI0 for dependent AI tasks
        } else {  // use AI trigger from primary device for other devices
            DAQmxErrChk (DAQmxCfgDigEdgeStartTrig(devices[i].aiHandle,AIline,DAQmx_Val_Rising));
        }

        // Setup AO Task
        DAQmxErrChk (setupAOTask(&devices[i], ao_channels, sampleRate, input_range, stim_samps_per_chan));
        if (i == 0) { // only get AO trigger and sample clock names and register done callback for primary device
            // Configure AO Start Trigger
            DAQmxErrChk (GetTerminalNameWithDevPrefix(devices[i].aoHandle,"ao/StartTrigger",AOtrigName));
            DAQmxErrChk (GetTerminalNameWithDevPrefix(devices[i].aoHandle,"RTSI1",AOline));
            DAQmxErrChk (DAQmxConnectTerms(AOtrigName,AOline,DAQmx_Val_DoNotInvertPolarity));  // connect AO start trigger to RTSI1 for dependent AO tasks
            ...
            DAQmxErrChk (GetTerminalNameWithDevPrefix(devices[i].aoHandle,"RTSI2",DOline));
            DAQmxErrChk (DAQmxConnectTerms(AOSampClkName,DOline,DAQmx_Val_DoNotInvertPolarity));  // connect AO sample clock to RTSI2 for dependent DO stim tasks
            // Register Done Callback for AO task
            DAQmxErrChk (DAQmxRegisterDoneEvent(devices[i].aoHandle,0,StimDoneCallback,NULL));
        } else {  // use AO trigger from primary device for other devices
            DAQmxErrChk (DAQmxCfgDigEdgeStartTrig(devices[i].aoHandle,AOline,DAQmx_Val_Rising));
        }
        ...
        for (int ch = 0; ch < TOTAL_CHANS; ch++) {   // generate stimulation waveform
            if (devices[i].ao_enable[ch]) {
                ...
                for (int i = 0; i < stim_samps_per_chan; i++) {
                    stim_data[i * TOTAL_CHANS + ch] = 0.0; // fill disabled AO channels with 0V
                }
                if (ref_stim_wave == NULL) {
                    ref_stim_wave = (double*)calloc(stim_samps_per_chan, sizeof(double));
                    ref_samps = stim_samps_per_chan;
                }
            }
        }
        ...
        DAQmxErrChk (DAQmxWriteAnalogF64(devices[i].aoHandle,
                                         devices[i].stimSamps,
                                         0,
                                         10.0,
        ...
        if (i != 0) { // start secondary device AI task first
            DAQmxErrChk (DAQmxStartTask(devices[i].aiHandle));
        }
    }
    // Start primary device AI task
    DAQmxErrChk (DAQmxStartTask(devices[0].aiHandle));
...
Error:
    ...
    for (size_t i = 0; i < num_devices; i++) {
        ...
        if(devices[i].aiHandle) {
            DAQmxStopTask(devices[i].aiHandle);
            DAQmxClearTask(devices[i].aiHandle);
            devices[i].aiHandle = 0;
        }
        // Stop and clear AO tasks
        if(devices[i].aoHandle) {
            DAQmxStopTask(devices[i].aoHandle);
            DAQmxClearTask(devices[i].aoHandle);
            devices[i].aoHandle = 0;
        }
        // Stop and clear stim DO tasks
        if(devices[i].stimDOHandle) {
            DAQmxStopTask(devices[i].stimDOHandle);
            DAQmxClearTask(devices[i].stimDOHandle);
            devices[i].stimDOHandle = 0;
        }
        // Stop and clear baseline DO tasks
        if(devices[i].doHandle) {
            DAQmxStopTask(devices[i].doHandle);
            DAQmxClearTask(devices[i].doHandle);
...
static int32 setupAITask(DeviceConfig *dev, const char physicalChannels[],
                        float64 sampleRate, float64 input_range, uInt64 sampsPerChan)
{
    int32 error = 0;

    DAQmxErrChk(DAQmxCreateTask("", &dev->aiHandle));
    DAQmxErrChk(DAQmxCreateAIVoltageChan(dev->aiHandle,
                                         physicalChannels,
                                         "",
                                         DAQmx_Val_Diff,
                                         ...
                                         DAQmx_Val_Volts,
                                         NULL));
    DAQmxErrChk(DAQmxSetRefClkSrc(dev->aiHandle, "PXI_Clk10"));
    DAQmxErrChk(DAQmxSetRefClkRate(dev->aiHandle, 10000000.0));
    DAQmxErrChk(DAQmxCfgSampClkTiming(dev->aiHandle,
                                      "",
                                      sampleRate,
                                      DAQmx_Val_Rising,
                                      DAQmx_Val_ContSamps,
	...
	if( DAQmxFailed(error) ) {
        ...
        for (int i = 0; i < num_devices; i++) {
            ...
            DAQmxErrChk (setupDOTask(&devices[i],
                                    ...
                                    false));
            DAQmxErrChk (DAQmxStopTask(devices[i].doHandle)); // Stop task
            DAQmxErrChk (DAQmxClearTask(devices[i].doHandle)); // Clear task
            devices[i].doHandle = 0;
        }
	}
	return 0;
...
static int32 setupAOTask(DeviceConfig *dev, const char physicalChannels[],
                        float64 sampleRate, float64 input_range, uInt64 sampsPerChan)
{
    int32 error = 0;

    DAQmxErrChk(DAQmxCreateTask("", &dev->aoHandle));
    DAQmxErrChk(DAQmxCreateAOVoltageChan(dev->aoHandle,
                                         physicalChannels,
                                         "",
                                         -input_range, input_range,
                                         DAQmx_Val_Volts,
                                         NULL));
    DAQmxErrChk(DAQmxSetRefClkSrc(dev->aoHandle, "PXI_Clk10"));
    DAQmxErrChk(DAQmxSetRefClkRate(dev->aoHandle, 10000000.0));
    DAQmxErrChk(DAQmxCfgSampClkTiming(dev->aoHandle,
                                      "",
                                      sampleRate,
                                      DAQmx_Val_Rising,
                                      DAQmx_Val_FiniteSamps,
                                      sampsPerChan));
    DAQmxErrChk(DAQmxSetWriteRegenMode(dev->aoHandle, DAQmx_Val_AllowRegen));
Error:
    return error;
}
...
static int32 CVICALLBACK StimDoneCallback(TaskHandle taskHandle, int32 status, void *callbackData)
{
    ...
    // For each device, stop AO & stim DO (if running), then rewrite the DO stim data
    for (int i = 0; i < num_devices; i++) {
        if (devices[i].aoHandle) {
            DAQmxErrChk (DAQmxStopTask(devices[i].aoHandle));
        }
        if (devices[i].stimDOHandle && i == stim_target) {
            DAQmxErrChk (DAQmxStopTask(devices[i].stimDOHandle));
            // Load stim DO data for next stimulation
            int32 doSampsWritten = 0;
            DAQmxErrChk (DAQmxWriteDigitalU32(devices[i].stimDOHandle,
                                              devices[i].stimSamps,  
                                              0,  		// autostart = false
                                              10.0,
                                              DAQmx_Val_GroupByChannel,
...
Error:
    if(DAQmxFailed(error)) {
        DAQmxGetExtendedErrorInfo(errBuff,2048);
        printf("DAQmx Error in StimDoneCallback: %s\n", errBuff);
    }
    return 0;
}

static int32 setupDOTask(DeviceConfig *dev, const char physicalChannels[],
						 int32 sampsPerChan, bool32 autoStart, uInt32 *data,
						 int32 *sampsWritten, float64 sampleRate, bool isStimTask)
{
    int32 error = 0;
	...
	if (isStimTask && sampsPerChan > 1) {
		DAQmxErrChk(DAQmxCfgSampClkTiming(*taskHandle,
										  DOline,      // use AO sample clock for timing
										  sampleRate,
										  DAQmx_Val_Rising,
										  DAQmx_Val_FiniteSamps,
										  sampsPerChan));
...
void save_to_hdf5(hid_t dataset_name, double *data, int numSamps, int numChans) {
	...
	hsize_t new_dims[2] = {curr_dims[0] + numSamps, numChans};
	...
	hsize_t count[2] = {numSamps, numChans};
