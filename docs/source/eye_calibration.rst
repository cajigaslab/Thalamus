...
* **Projective** -- a 6-parameter projective (homography) mapping from raw eye
  ...
  the collected fixation/target pairs; the mapping always sends the raw origin to
  the screen origin exactly (no offset/bias term).  *Use this* when a single global
  mapping fits the data well -- it is the simplest, most common choice.
* **Angular Scaling** -- a polar model: the raw signal's angle selects an
  ...
  vary by direction (independent X/Y behavior).  Each pin carries a list of
  **notches** -- (eye magnitude, screen magnitude) pairs -- so the scale can also
  vary with eccentricity along that direction, not just by direction.
  Interpolation wraps around the circle, and a **Scale Default** is used before any
  pins are set.  *Use this* when the eye signal is non-uniform across directions
  and a single projective fit leaves systematic error.
...
1. **Show targets.** Add saccade targets in the operator view (right-click for the
   context menu) and cycle through them so the subject fixates each in turn.  The
   context menu's **Presets** submenu can lay out a full set of targets in one step
   -- **8 Targets** (evenly spaced around a circle), **4 Targets On Axes**
...
4. **Refine.** For Angular Scaling you can **nudge** pins by dragging to locally
   ...
   eccentricity breakpoint along a pin; **undo/redo** is available for every
   change, and nudges/added notches survive a subsequent re-Fit instead of being
   overwritten.
5. **Reset / Clear.** **Reset** restores the model to defaults; **Clear** wipes the
   accumulated gaze trace without discarding the calibration; **Recenter** sends a
   recenter request to the OCULOMATIC node feeding this calibration.

The operator view also overlays a magenta **grid**: a regular mesh of eye-space
points mapped through the current model into screen space, so you can see at a
glance how the fit warps eye coordinates across the field -- useful for spotting
non-uniform distortion. The currently-selected saccade target is drawn with a
bold, fully-opaque outline so it's easy to tell apart from the other configured
targets. The operator view shows an **"Unfocused"** overlay whenever it doesn't
have keyboard focus, as a reminder that the keyboard shortcuts below won't fire
...
* **Recenter** -- send a recenter request to the OCULOMATIC node driving this
  calibration.
...
* **Fixation Radius** / **Saccade Radius** -- on-screen sizes of the fixation point
  and the saccade targets.  Persisted with the configuration.
* **Reward (ms)** and **Reward Node** -- duration of the reward pulse and the name of
  ...
  behavior during calibration.  ``Reward (ms)`` is persisted with the configuration.
* **Eye Opacity** -- a slider (0-255) controlling the alpha/transparency of the
  gaze-trace overlay in the operator view.  This is a display-only setting, local to
  the current session (not saved with the configuration).
