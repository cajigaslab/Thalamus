...
struct RemoteNode::Impl {
  ...
  std::unique_ptr<grpc::ClientReaderWriter<thalamus_grpc::RemoteNodeMessage,
  ...
  std::unique_ptr<::grpc::ClientAsyncReaderWriter<
      ::thalamus_grpc::StimRequest, ::thalamus_grpc::StimResponse>>
      stim_stream;
  ...
  queue_stim(thalamus_grpc::StimRequest &&request) {
  ...
  void send_stim() {
    if (stim_ready && !stim_queue.empty()) {
      ...
      stim_stream->Write(stim_request,
                         reinterpret_cast<void *>(STIM_NODE_WRITE));
      stim_ready = false;
    }
  }
  ...
  void grpc_target(std::shared_ptr<grpc::Channel> channel, std::string node,
    ...
    while (running) {
      ...
      *analog_request.mutable_node() = selector;
      auto analog_stream =
          stub->Asyncanalog(&analog_context, analog_request, queue.get(),
                            reinterpret_cast<void *>(ANALOG_CONNECT));
      auto xsens_stream =
      ...
      while (running) {
        ...
        switch (tag) {
        case ANALOG_CONNECT:
          analog_stream->Read(&analog_response,
                              reinterpret_cast<void *>(ANALOG_READ));
          break;
        case XSENS_CONNECT:
        ...
        case STIM_CONNECT: {
          ...
          *request.mutable_node() = selector;
          ...
          send_stim();
          stim_stream->Read(&stim_response,
                            reinterpret_cast<void *>(STIM_READ));
        } break;
        ...
        case ANALOG_READ: {
          ...
          sample_intervals.resize(
              size_t(analog_response.sample_intervals_size() + 2));
          std::transform(analog_response.sample_intervals().begin(),
                         analog_response.sample_intervals().end(),
                         sample_intervals.begin() + 2, [](const auto &s) {
        ...
        case IMAGE_READ: {
          ...
          time = std::chrono::steady_clock::now().time_since_epoch();
          for(auto i = 0;i < image_response.data_size();++i) {
            const auto& data = image_response.data(i);
            while(i >= planes.size()) {
              planes.emplace_back();
            }
            planes[i].insert(planes[i].end(), data.begin(), data.end());
          }
          num_planes = image_response.data_size();
    ...
    // analog_stream->Finish(&ok_status,
    // reinterpret_cast<void*>(ANALOG_FINISH)); xsens_stream->Finish(&ok_status,
    // reinterpret_cast<void*>(XSENS_FINISH)); ping_stream->Finish(&ok_status,
    // reinterpret_cast<void*>(PING_FINISH)); auto analog_finished = false; auto
    // xsens_finished = false; auto ping_finished = false;
  ...
  void on_change(ObservableCollection::Action,
    ...
    if (key_str == "Address") {
      ...
      auto target = [this, new_channel,
                     c_remote_node_name = this->remote_node_name,
                     c_ping_interval = this->configured_ping_interval,
                     c_probe_size = this->configured_probe_size] {
        grpc_target(new_channel, c_remote_node_name, c_ping_interval,
                    size_t(c_probe_size));
      };
...
RemoteNode::RemoteNode(ObservableDictPtr state,
                       boost::asio::io_context &io_context,
                       NodeGraph *node_graph)
...
std::chrono::nanoseconds RemoteNode::time() const { return impl->time; }
...
std::span<MotionCaptureNode::Segment const> RemoteNode::segments() const {
  return std::span<MotionCaptureNode::Segment const>(impl->segments.begin(),
                                                     impl->segments.end());
}
const std::string_view RemoteNode::pose_name() const { return impl->pose_name; }
void RemoteNode::inject(const std::span<Segment const> &) {
  THALAMUS_ASSERT(false, "RemoteNode::inject unimplemented.");
}
bool RemoteNode::has_motion_data() const { return impl->has_xsens_data; }
bool RemoteNode::has_analog_data() const { return impl->has_analog_data; }
...
std::future<thalamus_grpc::StimResponse>
RemoteNode::stim(thalamus_grpc::StimRequest &&request) {
...
ImageNode::Plane RemoteNode::plane(int i) const {
  return Plane(impl->planes[i].begin(), impl->planes[i].end());
}
