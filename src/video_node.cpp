...
namespace thalamus {
  ...
  struct VideoNode::Impl {
    ...
    Impl(ObservableDictPtr state, boost::asio::io_context& io_context, NodeGraph* graph, VideoNode* outer)
      : state(state)
      , outer(outer)
    ...
    void ffmpeg_target(const std::string input_name) {
      ...
      auto err = avformat_open_input(&context.format_context, input_name.c_str(), nullptr, &context.options);
      if (err < 0) {
      ...
      auto stream_index = av_find_best_stream(context.format_context, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
      avcodec_parameters_to_context(context.codec, context.format_context->streams[stream_index]->codecpar);
      context.codec->pkt_timebase = context.format_context->streams[stream_index]->time_base;
      auto time_base = context.codec->pkt_timebase;
      auto av_frame_rate = context.format_context->streams[stream_index]->avg_frame_rate;
      ...
      while(running) {
        ...
        while(true) {
          ...
          auto new_bps = std::accumulate(time_bits.begin(), time_bits.end(), 0.0, [](auto a, auto b) { return a + b.second; });
    ...
    void start() {
      std::string filename = state->contains("File Name") ? state->at("File Name") : std::string();

      ffmpeg_thread = std::thread(std::bind(&Impl::ffmpeg_target, this, filename));
    }
  ...
  VideoNode::VideoNode(ObservableDictPtr state, boost::asio::io_context& io_context, NodeGraph* graph)
    : impl(new Impl(state, io_context, graph, this)) {}

  VideoNode::~VideoNode() {}

  std::string VideoNode::type_name() {
    return "VIDEO";
  }
  ...
  size_t VideoNode::num_planes() const {
    return impl->data.size();
  }

  ImageNode::Format VideoNode::format() const {
    return impl->format;
  }

  size_t VideoNode::width() const {
    return impl->width;
  }

  size_t VideoNode::height() const {
    return impl->height;
  }

  void VideoNode::inject(const thalamus_grpc::Image& image) {
    THALAMUS_ASSERT(false);
  }

  std::chrono::nanoseconds VideoNode::time() const {
    return impl->time;
  }
  ...
  int VideoNode::num_channels() const {
    return 2;
  }

  std::chrono::nanoseconds VideoNode::sample_interval(int channel) const {
    return impl->frame_interval;
  }
  ...
  void VideoNode::inject(const thalamus::vector<std::span<double const>>& data, const thalamus::vector<std::chrono::nanoseconds>& interval, const thalamus::vector<std::string_view>& names) {
    impl->has_analog = true;
    impl->has_image = false;
  }

  bool VideoNode::has_analog_data() const {
    return impl->has_analog;
  }

  bool VideoNode::has_image_data() const {
