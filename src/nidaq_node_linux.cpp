...
static std::optional<DeviceParams>
parse_device_string(const std::string &device) {
  std::regex regex("[a-zA-Z]+(\\d+)/([a-zA-Z]+)(\\d+)(:(\\d+))?(/"
                   "[a-zA-Z]+(\\d+)(:(\\d+))?)?$");
  std::smatch match_result;
...
struct NidaqNode::Impl {
  ...
  unsigned char *bytes_buffer =
      reinterpret_cast<unsigned char *>(lsampl_buffer);
  ...
  void store_sample(unsigned int sample) {
    auto physical_value =
        comedi_to_phys(sample, range_info[next_channel], maxdata[next_channel]);
  ...
  void on_timer_analog(std::chrono::milliseconds polling_interval,
    ...
    auto ret = read(comedi_fileno(device), bytes_buffer + offset,
                    buffer_size - offset);
    if (ret < 0) {
      if (errno == EAGAIN) {
  ...
  void on_change(ObservableCollection::Action,
    ...
    if (key_str == "Channel") {
      ...
      if (is_running) {
        ...
        auto filename =
            absl::StrFormat("/dev/comedi%d", device_params->device - 1);
        ...
        if (digital) {
          ...
          on_timer_digital(polling_interval, _sample_interval,
...
struct NidaqOutputNode::Impl {
  ...
  void on_change(ObservableCollection::Action,
    ...
    auto key_str = std::get<std::string>(k);
    if (key_str == "Running" || key_str == "Source" || key_str == "Channel" ||
        key_str == "Digital") {
      ...
      graph->get_node(source_str, [&](auto node) {
        ...
        source = locked_source ? node_cast<AnalogNode *>(locked_source.get())
                               : nullptr;
        if (!source) {
          source = nullptr;
          return;
  ...
  void on_data(Node *) {
    ...
    if (digital) {
      ...
      for (auto i = 0; i < source->num_channels(); ++i) {
        ...
        auto ret = comedi_dio_bitfield2(
            device, subdevice, std::numeric_limits<unsigned int>::max(), &bits,
            0);
        if (ret < 0) {
          THALAMUS_LOG(error) << strerror(errno);
          running = false;
      ...
      for (auto i = 0ull; i < count; ++i) {
        ...
        if (!data.empty()) {
          auto sample =
              comedi_from_phys(data.back(), range_info[i], maxdata[i]);
          comedi_data_write(device, subdevice, channel, 0, AREF_GROUND, sample);
        }
      }
    }
  }
...
std::string NidaqOutputNode::type_name() { return "NIDAQ_OUT (COMEDI)"; }
