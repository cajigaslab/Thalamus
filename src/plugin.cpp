...
EXPORT int thalamus_start(const char *config_filename, const char *target_node,
                          int port, bool trace) {
#ifdef _WIN32
  timeBeginPeriod(1);
#endif
  ...
  auto log_absolute_filename =
      thalamus::get_home() / std::filesystem::path(log_filename);
  ...
  if (good_log_file) {
    ...
    std::cout << "Failed to create log file: " << log_absolute_filename
              << std::endl;
  }
  ...
  if (config_filename != nullptr) {
    ...
    json_buffer.reset(new std::stringstream(
        "{\"nodes\":[{\"type\":\"ANALOG\", \"name\":\"ANALOG\"}]}"));
  }
  target_node = target_node ? target_node : "ANALOG";
  auto parsed = boost::json::parse(*json_buffer);
  auto state = std::get<thalamus::ObservableDictPtr>(
      thalamus::ObservableCollection::from_json(parsed));
  ...
  node_graph.reset(new thalamus::NodeGraphImpl(nodes, io_context, system_start,
                                               steady_start));
  service.reset(
      new thalamus::Service(std::make_shared<thalamus::ObservableDict>(),
                            io_context, *node_graph, ""));
  node_graph->set_service(service.get());
  builder.RegisterService(service.get());
  server = builder.BuildAndStart();

  THALAMUS_LOG(info) << "Server listening on " << server_address << std::endl;
  grpc_thread = std::thread([] { server->Wait(); });

  if (!good_log_file) {
    service->warn("Log creation failed",
                  std::string("Unable to create log file ") +
                      log_absolute_filename.string());
  }

  work_guard.reset(new WorkGuard{io_context.get_executor()});
  main_thread = std::thread([] { io_context.run(); });
  ...
  boost::asio::post(io_context, [&] {
    ...
    connection = node_graph->get_node_scoped(
        selector, [&](std::weak_ptr<thalamus::Node> node) {
...
EXPORT int thalamus_push(size_t num_channels, const double *samples,
                         const size_t *counts,
