...
#endif
    x_denominator =
        x_denominator == 0 ? dimensions.first - 2 * x_gain : x_denominator;
    y_denominator =
...
#pragma clang diagnostic pop
    ...
    if (invert_x && invert_y) {
      result =
          std::make_pair((center_pix_x - pix_x) / x_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_x,
                         (center_pix_y - pix_y) / y_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_y);
    } else if (invert_x && !invert_y) {
      result =
          std::make_pair((center_pix_x - pix_x) / x_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_x,
                         (pix_y - center_pix_y) / y_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_y);
    } else if (!invert_x && invert_y) {
      result =
          std::make_pair((pix_x - center_pix_x) / x_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_x,
                         (center_pix_y - pix_y) / y_denominator * AOUT_RANGE +
                             AOUT_MIN - center_offset_y);
    } else {
  ...
  void on_data(Node *) {
    ...
    TRACE_EVENT_BEGIN("thalamus", "OculomaticNode::on_data",
    ...
    if (image_source->format() != ImageNode::Format::Gray ||
      ...
      cv::cvtColor(cv::Mat(height, width, CV_8UC3, data), in,
                   cv::COLOR_GRAY2RGB);
    ...
    pool.push([width, event_id, height, frame_id = next_input_frame++,
               ...
               this_invert_y = this->invert_y, this_threshold = this->threshold,
               moved_in = std::move(in),
               this_outer = outer->shared_from_this()] {
      TRACE_EVENT_BEGIN("thalamus", "OculomaticNode::compute",
                        perfetto::Flow::ProcessScoped(event_id));
      ...
      if (!this_computing) {
        ...
        boost::asio::post(this_io_context, [&this_current_result,
          ...
          this_output_frames[frame_id] =
              Result{0, 0, 0, *out, true, false, frame_interval, sample_time};
          this_mat_pool.insert(out);
          for (auto i = this_output_frames.begin();
               i != this_output_frames.end();) {
      ...
      }
      {
        ...
        cv::findContours(moved_in, contours, cv::RETR_EXTERNAL,
                         cv::CHAIN_APPROX_SIMPLE);
      }
      std::vector<double> areas(contours.size(), 0);
      {
        TRACE_EVENT("thalamus", "cv:contourArea");
        std::transform(contours.begin(), contours.end(), areas.begin(),
      ...
      for (size_t i = 0; i < contours.size(); ++i) {
        if (areas[i] <= max_area_pixels && min_area_pixels <= areas[i] &&
            (selected == -1 || areas[i] > areas[size_t(selected)])) {
          selected = int(i);
        }
      }
      ...
      if (selected > -1) {
        ...
        gaze = normalize_center(
            center.first, center.second, std::make_pair(width, height),
            this_centering_pix, this_centering_offset, this_x_gain, this_y_gain,
            this_invert_x, this_invert_y);
        cv::Rect bounds;
        {
          ...
          [&this_current_result, &this_next_output_frame, &this_output_frames,
           ...
           frame_interval, event_id, sample_time] {
            TRACE_EVENT("thalamus", "OculomaticNode Post Main",
                        perfetto::TerminatingFlow::ProcessScoped(event_id));
            this_output_frames[frame_id] =
                Result{gaze.first, gaze.second, diameter,       *out,
                       true,       true,        frame_interval, sample_time};
            this_mat_pool.insert(out);
            for (auto i = this_output_frames.begin();
                 i != this_output_frames.end();) {
              if (i->first == this_next_output_frame) {
...
OculomaticNode::OculomaticNode(ObservableDictPtr state,
                               boost::asio::io_context &io_context,
                               NodeGraph *graph)
...
std::string OculomaticNode::type_name() { return "OCULOMATIC"; }
...
size_t OculomaticNode::num_planes() const { return 1; }
...
std::span<const double> OculomaticNode::data(int channel) const {
  switch (channel) {
  case 0:
    return std::span<const double>(&impl->current_result.x,
                                   &impl->current_result.x + 1);
  case 1:
    return std::span<const double>(&impl->current_result.y,
                                   &impl->current_result.y + 1);
  case 2:
    return std::span<const double>(&impl->current_result.diameter,
                                   &impl->current_result.diameter + 1);
  default:
    return std::span<const double>();
  }
}

int OculomaticNode::num_channels() const { return 3; }
...
bool OculomaticNode::prepare() { return true; }
