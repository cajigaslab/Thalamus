...
//																									//
//	20.07.2018 NH: throw an exception if we can't get the ProcAddress								//
//	                                                                                                //
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
#define FUNC_LOADED m_FunctionLoaded
...
#define DECLARE_FUNCTION0(retVal, FuncName) \
	...
	{ \
		...
		{ \
			...
			{\
				std::ostringstream e; \
				e << "Failed to GetProcAddress for library function " << #FuncName; \
...
public:
	...
	bool SetDllName(const char* newName)
	{
		...
		if (!m_dllHandle)
		{
			...
			if (newName)
			{
				size_t ns = strlen(newName) + 1;
				m_dllName = new char[ns];
				//make sure memory was allocated
				if (m_dllName)
					strcpy_s(m_dllName, ns, newName);
				else
					retVal = false;
			}
			retVal = true;
