...
namespace thalamus {
  ...
  public:
    ...
    void on_change(ObservableCollection* source, ObservableCollection::Action, const ObservableCollection::Key& k, const ObservableCollection::Value& v) {
      ...
      if(key_str == "Source") {
        ...
        get_source_connection = graph->get_node_scoped(val_str, [&](std::weak_ptr<Node> weak) {
            auto lock = weak.lock();
        ...
        transform->recap(std::bind(&Impl::on_change, this, transform.get(), _1, _2, _3));
      }
    }
  };

  TouchScreenNode::TouchScreenNode(ObservableDictPtr state, boost::asio::io_context& io_context, NodeGraph* graph)
    : impl(new Impl(state, io_context, graph, this)) {}

  TouchScreenNode::~TouchScreenNode() {}

  std::string TouchScreenNode::type_name() {
    return "TOUCH_SCREEN";
  }
  ...
  std::span<const double> TouchScreenNode::data(int channel) const {
    if(channel == 0) {
      return std::span<const double>(&impl->output.first, &impl->output.first + 1);
    } else if (channel == 1) {
      return std::span<const double>(&impl->output.second, &impl->output.second + 1);
    } else {
      return std::span<const double>();
    }
  }

  int TouchScreenNode::num_channels() const {
    return 2;
  }
  ...
  void TouchScreenNode::inject(const thalamus::vector<std::span<double const>>&, const thalamus::vector<std::chrono::nanoseconds>&, const thalamus::vector<std::string_view>&) {
    THALAMUS_ASSERT(false);
  }
   
  bool TouchScreenNode::has_analog_data() const {
    return true;
  }

  size_t TouchScreenNode::modalities() const { return infer_modalities<TouchScreenNode>(); }
}
