    ...
    struct Cti {
      ...
      struct IntReg {
        ...
        long long int read() {
          ...
          auto result = little_endian
            ? std::accumulate(buffer.rbegin(), buffer.rend(), _unsigned || !(first_byte & 0x80) ? 0ll : -1ll, visitor)
        ...
        void write(long long int value) {
          ...
          error = cti->GCWritePort(handle, total_address, buffer.data(), &length);
          THALAMUS_ASSERT(error == GenTL::GC_ERR_SUCCESS);
        }
      };
      ...
      struct FloatReg {
        ...
        double read() {
          ...
          if(length == 4) {
            if(little_endian) {
              boost::endian::little_to_native_inplace(*reinterpret_cast<int*>(buffer.data()));
            } else {
            ...
            if(little_endian) {
              ...
              boost::endian::big_to_native_inplace(*reinterpret_cast<long long int*>(buffer.data()));
            }
      ...
      template<typename T>
      static std::optional<T> get_optional(const boost::property_tree::ptree& tree, const std::string& path) {
        auto text = tree.get_optional<std::string>(path);
        if(!text) {
          return std::nullopt;
        }
      ...
      struct DeviceImpl : public Device {
        ...
        bool load_xml() {
          ...
          for(auto l = 0u;l < num_urls;++l) {
            ...
            error = cti->GCGetPortURLInfo(port_handle, l, GenTL::URL_INFO_URL, &type, nullptr, &id_size);
            if(error != GenTL::GC_ERR_SUCCESS) {
            ...
            error = cti->GCGetPortURLInfo(port_handle, l, GenTL::URL_INFO_URL, &type, port_url.data(), &id_size);
            if(error != GenTL::GC_ERR_SUCCESS) {
              THALAMUS_LOG(info) << "GCGetPortURLInfo failed.";
              return false;
            }
            ...
            std::vector<std::string> tokens = absl::StrSplit(port_url, absl::ByAnyChar(":;"));
            ...
            while(offset < length) {
              ...
              if(signature == 0x04034b50) {
                ...
                offset += 4;
                auto min_version = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto bit_flag = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto compression = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto modification_time = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto modification_date = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto crc = boost::endian::little_to_native(*reinterpret_cast<unsigned int*>(buffer + offset));
                offset += 4;
                auto compressed_size = boost::endian::little_to_native(*reinterpret_cast<unsigned int*>(buffer + offset));
                offset += 4;
                auto uncompressed_size = boost::endian::little_to_native(*reinterpret_cast<unsigned int*>(buffer + offset));
                offset += 4;
                auto filename_length = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto extra_length = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto filename = std::string(reinterpret_cast<char*>(buffer + offset), filename_length);
                offset += filename_length;
                auto extra = std::vector<unsigned char>(buffer + offset, buffer + offset + extra_length);
                offset += extra_length;
                ...
                std::cout << "ZIP file = " << filename << " Compression = " << compression << std::endl;
                if(compression != 8) {
                  THALAMUS_LOG(info) << "Unsupported compression: " << compression;
                  return false;
                }
                ...
                offset += 4;
                auto made_version = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto min_version = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                offset += 2;
                auto bit_flag = boost::endian::little_to_native(*reinterpret_cast<short*>(buffer + offset));
                ...
                auto extra_length = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto comment_length = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto start_disk = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto internal_attr = boost::endian::little_to_native(*reinterpret_cast<unsigned short*>(buffer + offset));
                offset += 2;
                auto external_attr = boost::endian::little_to_native(*reinterpret_cast<unsigned int*>(buffer + offset));
                offset += 4;
                auto file_offset = boost::endian::little_to_native(*reinterpret_cast<unsigned int*>(buffer + offset));
                offset += 4;
                auto filename = std::string(reinterpret_cast<char*>(buffer + offset), filename_length);
                offset += filename_length;
                auto extra = std::vector<unsigned char>(buffer + offset, buffer + offset + extra_length);
                offset += extra_length;
                auto comment = std::string(reinterpret_cast<char*>(buffer + offset), comment_length);
        ...
        bool setup_registry() {
          std::vector<std::pair<std::string, const boost::property_tree::ptree*>> open;
          open.emplace_back("", &tree);
          while(!open.empty()) {
            auto [current_name, current] = open.back();
            open.pop_back();
            try {
              auto attributes = current->find("<xmlattr>");
              ...
              if(current_name == "Integer") {
                ...
                nodes[name] = StringReg {this, port_handle, cti, address, p_address, length};
                ...
                nodes[name] = IntReg {this, port_handle, cti, address, p_address, length, little_endian, _unsigned};
                ...
                auto little_endian = current->get<std::string>("Endianess", "LittleEndian") == "LittleEndian";
                auto _unsigned = current->get<std::string>("Sign", "Unsigned") == "Unsigned";
                nodes[name] = FloatReg {this, port_handle, cti, address, p_address, length, little_endian, _unsigned};
                ...
                for(auto& pair : *current) {
                  if(pair.first == "pVariable") {
                    ...
                    THALAMUS_ASSERT(name);
                    values[*name] = pair.second.data();
                  }
                }

                nodes[name] = IntConverter {this, port_handle, cti, 0, "", 0, p_value, formula_to, formula_from, values};
                ...
                nodes[name] = Converter {this, port_handle, cti, 0, "", 0, p_value, formula_to, formula_from, values};
                ...
                nodes[name] = IntSwissKnife {this, port_handle, cti, 0, "", 0, formula, values};
                ...
                nodes[name] = SwissKnife {this, port_handle, cti, 0, "", 0, formula, values};
                ...
                for(auto& pair : *current) {
                  ...
                  auto enum_name = pair.second.get_optional<std::string>("<xmlattr>.Name");
                  auto enum_value = get_optional<long long int>(pair.second, "Value");
        ...
        DeviceImpl(GenTL::DEV_HANDLE dev_handle, Cti* cti)
          ...
          for(auto i = 0u;i < num_streams;++i) {
            ...
            error = cti->GCRegisterEvent(ds_handle, GenTL::EVENT_NEW_BUFFER, &event_handle);
            if(error != GenTL::GC_ERR_SUCCESS) {
            ...
            size_t info_size;
            error = cti->EventGetInfo(event_handle, GenTL::EVENT_SIZE_MAX, &info_type, &buffer_size, &info_size);
            if(error != GenTL::GC_ERR_SUCCESS) {
        ...
        void start_stream(boost::asio::io_context& io_context) {
          ...
          buffer_handles.resize(1);
          buffer_data.resize(1);
        ...
        void stream_target(long long int width, long long int height) {
          while(streaming) {
            ...
            io_context->post([this, &buffer=buffer_data.at(index), handle=data.BufferHandle, width, height, now] {
              if(!streaming) {
                return;
              }
        ...
        void set(const std::string& reg, const std::variant<long long int, std::string, double>& value) {
          ...
          if (std::holds_alternative<Integer>(i->second)) {
            std::get<Integer>(i->second).write(variant_cast<long long int>(value));
          } else if (std::holds_alternative<IntReg>(i->second)) {
            std::get<IntReg>(i->second).write(variant_cast<long long int>(value));
          } else if (std::holds_alternative<IntConverter>(i->second)) {
            std::get<IntConverter>(i->second).write(variant_cast<long long int>(value));
          } else if (std::holds_alternative<Float>(i->second)) {
            ...
            std::get<Enumeration>(i->second).write(variant_cast<std::string>(value));
          } else {
            THALAMUS_ASSERT(false);
          }
        }
...
#endif
        if(!library_handle) {
          THALAMUS_LOG(info) << "Couldn't find " << path << ".  " << name << " disabled";
          return;
        }
...
#define LOAD_FUNC(name) name = load_function<GenTL::P##name>(#name);if(!name) { return; }
        ...
        if(error != GenTL::GC_ERR_SUCCESS) {
          THALAMUS_LOG(info) << "TLUpdateInterfaceList failed.  " << name << " disabled";
          return;
        }
        ...
        if(error != GenTL::GC_ERR_SUCCESS) {
          THALAMUS_LOG(info) << "TLGetNumInterfaces failed.  " << name << " disabled";
          return;
        }
        ...
        for(auto i = 0u;i < num_interfaces;++i) {
          ...
          if(error != GenTL::GC_ERR_SUCCESS) {
            THALAMUS_LOG(info) << "TLGetInterfaceID failed.  " << name << " disabled";
            return;
          }
          ...
          GenTL::INFO_DATATYPE type;
          std::string tltype;
          error = TLGetInterfaceInfo(tl_handle, interface_id.c_str(), GenTL::INTERFACE_INFO_TLTYPE, &type, nullptr, &id_size);
          if(error != GenTL::GC_ERR_SUCCESS) {
            THALAMUS_LOG(info) << "TLGetInterfaceInfo failed.  " << name << " disabled";
            return;
          }
          tltype.resize(id_size);

          error = TLGetInterfaceInfo(tl_handle, interface_id.c_str(), GenTL::INTERFACE_INFO_TLTYPE, &type, tltype.data(), &id_size);
          if(error != GenTL::GC_ERR_SUCCESS) {
            THALAMUS_LOG(info) << "TLGetInterfaceInfo failed.  " << name << " disabled";
            return;
          }
          ...
          if(tltype == TLTypeU3VName) {
            ...
            if(error != GenTL::GC_ERR_SUCCESS) {
              THALAMUS_LOG(info) << "TLOpenInterface failed.  " << name << " disabled";
              return;
            }

            error = IFUpdateDeviceList(if_handle, &changed, 1000);
            if(error != GenTL::GC_ERR_SUCCESS) {
              THALAMUS_LOG(info) << "IFUpdateDeviceList failed.  " << name << " disabled";
              return;
            }
            ...
            if(error != GenTL::GC_ERR_SUCCESS) {
              THALAMUS_LOG(info) << "IFGetNumDevices failed.  " << name << " disabled";
              return;
            }
            ...
            for(auto j = 0u;j < num_devices;++j) {
              ...
              if(error != GenTL::GC_ERR_SUCCESS) {
                THALAMUS_LOG(info) << "IFOpenDevice failed.  " << name << " disabled";
                return;
              }

              auto device = std::make_shared<DeviceImpl>(dev_handle, this);
              auto var = device->get("DeviceID");
              THALAMUS_ASSERT(std::holds_alternative<std::string>(var));
              devices[std::get<std::string>(var)] = std::move(device);
            }
          }
        }
        if(!if_handle) {
          THALAMUS_LOG(info) << "No suitable interface found.  " << name << " disabled";
          return;
        }

        THALAMUS_LOG(info) << "Successfully loaded " << path << ".  " << name << " enabled";
        loaded = true;
      }
      ~Cti() {
        if(if_handle) {
    ...
    void initialize_camera(bool apply_state = false) {
      ...
      for(auto& cti : ctis) {
        ...
        if(i != cti->devices.end()) {
          ...
          frame_connection = this->device->frame_ready.connect(std::bind(&Impl::on_frame_ready, this, _1, _2, _3, _4));
          break;
        } 
        (*state)["Camera"].assign(default_camera);
        return;
      ...
      if(apply_state && state->contains("Height")) {
        ...
        long long int value = variant_cast<long long int>(this->device->get("Height"));
      ...
      if(apply_state && state->contains("OffsetX")) {
        ...
        long long int value = variant_cast<long long int>(this->device->get("OffsetX"));
      ...
      if(apply_state && state->contains("OffsetY")) {
        ...
        long long int value = variant_cast<long long int>(this->device->get("OffsetY"));
      ...
      if(apply_state && state->contains("AcquisitionFrameRate")) {
        ...
        auto value = variant_cast<double>(this->device->get("AcquisitionFrameRate"));
        target_framerate = value;
  ...
  std::string GenicamNode::type_name() {
    return "GENICAM";
  }
  ...
  size_t GenicamNode::width() const {
    return impl->width;
  }

  size_t GenicamNode::height() const {
    return impl->height;
  }

  void GenicamNode::inject(const thalamus_grpc::Image& image) {
    THALAMUS_ASSERT(false);
  }

  std::chrono::nanoseconds GenicamNode::time() const {
    return impl->time;
  }
