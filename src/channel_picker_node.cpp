...
public:
  ...
  std::vector<std::tuple<std::weak_ptr<AnalogNode>, int, std::string,
  ...
  void on_source_mappings_change(std::weak_ptr<AnalogNode> node,
    ...
    if (a == ObservableCollection::Action::Set) {
      ...
      single_source_map_connections.push_back(v_dict->changed.connect(
          std::bind(&Impl::on_source_mapping_change, this, node, k_int, v_dict,
                    _1, _2, _3)));
      v_dict->recap(std::bind(&Impl::on_source_mapping_change, this, node,
                              k_int, v_dict, _1, _2, _3));
    }
  }
  ...
  void on_sources_change(ObservableCollection::Action a,
    ...
    if (a == ObservableCollection::Action::Set) {
      ...
      graph->get_node(node_name, [&, node_name, v_list](auto node) {
        ...
        source_mapping_connections[node_name] =
            v_list->changed.connect(std::bind(&Impl::on_source_mappings_change,
                                              this, weak_analog, _1, _2, _3));
              ...
              if (!names_collected.contains(current_node)) {
                if (current_node) {
                  for (auto i = 0; i < current_node->num_channels(); ++i) {
                    ...
                    std::string channel_name(channel_name_view.data(),
                                             channel_name_view.size());
                    ...
                    if (static_cast<size_t>(i) >= v_list->size()) {
                      ...
                      mappings.emplace_back(weak_analog, i,
...
ChannelPickerNode::ChannelPickerNode(ObservableDictPtr state,
...
std::string ChannelPickerNode::type_name() { return "CHANNEL_PICKER"; }
...
std::span<const double> ChannelPickerNode::data(int channel) const {
  ...
  return locked.get() == impl->current_node &&
                 in_channel < impl->current_node->num_channels()
             ? locked->data(std::get<1>(pair))
...
size_t ChannelPickerNode::modalities() const {
  return infer_modalities<ChannelPickerNode>();
}
} // namespace thalamus
