...
static unsigned int pos_of_char(const char chr) {
  ...
  if (chr >= 'A' && chr <= 'Z')
    return static_cast<unsigned int>(chr - 'A');
  else if (chr >= 'a' && chr <= 'z')
    return static_cast<unsigned int>(chr - 'a' + ('Z' - 'A') + 1);
  else if (chr >= '0' && chr <= '9')
    return static_cast<unsigned int>(chr - '0' + ('Z' - 'A') + ('z' - 'a') + 2);
  else if (chr == '+' || chr == '-')
    ...
    //
    THALAMUS_ASSERT(false, "Input is not valid base64-encoded data.");
}
