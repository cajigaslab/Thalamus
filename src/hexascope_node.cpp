...
struct HexascopeNode::Impl {
  ...
  std::vector<std::pair<std::chrono::nanoseconds, boost::qvm::quat<float>>>
      field_rotations;
  std::vector<std::pair<std::chrono::nanoseconds, boost::qvm::vec<float, 3>>>
  ...
  struct PortGuard {
    ...
    PortGuard(boost::asio::serial_port &port)
        : port(port), was_open(port.is_open()) {
      if (!was_open) {
  ...
  Impl(ObservableDictPtr state, boost::asio::io_context &io_context,
       HexascopeNode *outer, NodeGraph *graph)
      : io_context(io_context), state(state), outer(outer), graph(graph),
        pool(graph->get_thread_pool()), timer(io_context),
        orient_timer(io_context), port(io_context) {
  ...
  std::array<Accumulator *, 2> accumulators{&objective_accumulator,
                                            &field_accumulator};
  ...
  boost::asio::awaitable<void> go_home() { co_await move(0, 0, 0, 0, 0, 0); }
  boost::asio::awaitable<void> rotate(float theta, float phi, float psi) {
    auto current_pose = co_await read_hexa_pose_raw();
    co_await move(current_pose[0], current_pose[1], current_pose[2], theta, phi,
                  psi);
  }
  ...
  boost::asio::awaitable<void> home() {
    ...
    while (true) {
      for (auto point : points) {
        co_await move(std::get<0>(point), std::get<1>(point),
                      std::get<2>(point), std::get<3>(point),
                      std::get<4>(point), std::get<5>(point));
        ...
        for (auto i = 0; i < 6; ++i) {
          ...
          co_await boost::asio::async_write(
              port, boost::asio::buffer(buffer, 6), boost::asio::use_awaitable);
          co_await boost::asio::async_read(port,
                                           boost::asio::buffer(buffer, 20),
  ...
  bool reading = false;
  boost::asio::awaitable<void> move(float x, float y, float z, float theta,
                                    float phi, float psi) {
    co_await move_inner(x, y, z, theta, phi, psi, false);
  }
  boost::asio::awaitable<void> move_with_rehoming(float x, float y, float z,
                                                  float theta, float phi,
                                                  float psi) {
    co_await move_inner(x, y, z, theta, phi, psi, true);
  }

  boost::asio::awaitable<void> move_inner(float x, float y, float z,
                                          float theta, float phi, float psi,
                                          bool rehoming) {
    open_port();
    auto message_size = 38;
    // Allocate float array then reinterpret as a byte array so the bytes are
    // float aligned
    float floats[2   // header (8 bytes)
                 + 6 // orientation (6 floats)
                 + 2 // tail (need 6 bytes so allocate 2 floats, which gets 8
                     // bytes)
    ];
    ...
    floats[6] = phi;
    floats[7] = psi;
    THALAMUS_LOG(info) << "Moving to " << floats[2] << " " << floats[3] << " "
                       << floats[4] << " " << floats[5] << " " << floats[6]
                       << " " << floats[7] << " ";
    co_await boost::asio::async_write(port,
    ...
    while (moving) {
      ...
      for (auto i = 0; i < 6; ++i) {
        ...
        co_await boost::asio::async_write(port, boost::asio::buffer(buffer, 6),
                                          boost::asio::use_awaitable);
        co_await boost::asio::async_read(port, boost::asio::buffer(buffer, 20),
                                         boost::asio::use_awaitable);
        leg.load_leg_response(buffer);
  ...
  template <typename T>
  boost::asio::awaitable<std::optional<Pose>>
  measure_pose(T duration, Accumulator &accumulator) {
    ...
    for (auto i = 0ull; i < accumulator.position.size(); ++i) {
      float distance =
          boost::qvm::mag_sqr(accumulator.position[i] - position_mean);
  ...
  template <typename T>
  boost::asio::awaitable<std::optional<Pose>>
  measure_objective_pose(T duration) {
  ...
  boost::asio::awaitable<std::array<float, 6>> read_hexa_pose_raw() {
    ...
    co_await boost::asio::async_write(port, boost::asio::buffer(buffer, 6),
                                      boost::asio::use_awaitable);
    co_await boost::asio::async_read(port, boost::asio::buffer(buffer, 32),
                                     boost::asio::use_awaitable);

    co_return std::array<float, 6>{floats[2], floats[3], floats[4],
  ...
  boost::asio::awaitable<Pose> read_hexa_pose() {
    ...
    auto cv_quaternion = cv::Quat<float>::createFromEulerAngles(
  ...
  boost::qvm::mat<float, 4, 4> quat_to_mat(const boost::qvm::quat<float> &arg) {
    cv::Quat<float> quat(boost::qvm::S(arg), boost::qvm::X(arg),
                         boost::qvm::Y(arg), boost::qvm::Z(arg));
  ...
  boost::asio::awaitable<void> align() {
    ...
    auto camera_position =
        hexa_to_camera * boost::qvm::XYZ1(hexa_pose.position);
    ...
    auto field_z_axis =
        field_pose.rotation * boost::qvm::vec<float, 3>{1, 0, 0};
    ...
    auto rotation_axis =
        boost::qvm::cross(boost::qvm::XYZ(camera_z_axis), field_z_axis);
    auto rotation_angle = acos(z_dot);
    auto camera_rotation_vector = rotation_angle * rotation_axis;
    auto hexa_rotation_vector =
        camera_to_hexa * boost::qvm::XYZ1(camera_rotation_vector);

    cv::Vec3f rvec{boost::qvm::X(hexa_rotation_vector),
                   boost::qvm::Y(hexa_rotation_vector),
                   boost::qvm::Z(hexa_rotation_vector)};
    auto quaternion = cv::Quat<float>::createFromRvec(rvec);
    auto eulerAngles =
        quaternion.toEulerAngles(cv::QuatEnum::EulerAnglesType::EXT_XYZ);
    eulerAngles *= -180 / M_PI;

    co_await rotate(eulerAngles[0], eulerAngles[1], eulerAngles[2]);

    auto objective_to_field =
        field_pose.position - boost::qvm::XYZ(camera_position);

    auto camera_x_axis = camera_rot * boost::qvm::vec<float, 4>{1, 0, 0, 1};
    auto camera_y_axis = camera_rot * boost::qvm::vec<float, 4>{0, 1, 0, 1};

    auto x =
        boost::qvm::dot(objective_to_field, boost::qvm::XYZ(camera_x_axis));
    auto y =
        boost::qvm::dot(objective_to_field, boost::qvm::XYZ(camera_y_axis));
  ...
  boost::asio::awaitable<void> calibrate() {
    ...
    try {
      ...
      THALAMUS_LOG(info) << "camera_origin_pose "
                         << boost::qvm::X(camera_origin_pose.position) << " "
                         << boost::qvm::Y(camera_origin_pose.position) << " "
                         << boost::qvm::Z(camera_origin_pose.position);
      auto hexa_origin_pose = co_await read_hexa_pose();
      THALAMUS_LOG(info) << "hexa_origin_pose "
                         << boost::qvm::X(hexa_origin_pose.position) << " "
                         << boost::qvm::Y(hexa_origin_pose.position) << " "
                         << boost::qvm::Z(hexa_origin_pose.position);
      ...
      THALAMUS_LOG(info) << "camera_x_pose "
                         << boost::qvm::X(camera_x_pose.position) << " "
                         << boost::qvm::Y(camera_x_pose.position) << " "
                         << boost::qvm::Z(camera_x_pose.position);
      auto hexa_x_pose = co_await read_hexa_pose();
      THALAMUS_LOG(info) << "hexa_x_pose "
                         << boost::qvm::X(hexa_x_pose.position) << " "
                         << boost::qvm::Y(hexa_x_pose.position) << " "
                         << boost::qvm::Z(hexa_x_pose.position);
      ...
      THALAMUS_LOG(info) << "camera_y_pose "
                         << boost::qvm::X(camera_y_pose.position) << " "
                         << boost::qvm::Y(camera_y_pose.position) << " "
                         << boost::qvm::Z(camera_y_pose.position);
      auto hexa_y_pose = co_await read_hexa_pose();
      THALAMUS_LOG(info) << "hexa_y_pose "
                         << boost::qvm::X(hexa_y_pose.position) << " "
                         << boost::qvm::Y(hexa_y_pose.position) << " "
                         << boost::qvm::Z(hexa_y_pose.position);
      ...
      THALAMUS_LOG(info) << "camera_z_pose "
                         << boost::qvm::X(camera_z_pose.position) << " "
                         << boost::qvm::Y(camera_z_pose.position) << " "
                         << boost::qvm::Z(camera_z_pose.position);
      auto hexa_z_pose = co_await read_hexa_pose();
      THALAMUS_LOG(info) << "hexa_z_pose "
                         << boost::qvm::X(hexa_z_pose.position) << " "
                         << boost::qvm::Y(hexa_z_pose.position) << " "
                         << boost::qvm::Z(hexa_z_pose.position);
      ...
      THALAMUS_LOG(info) << "  " << boost::qvm::A<0, 0>(camera_to_hexa) << " "
                         << boost::qvm::A<0, 1>(camera_to_hexa) << " "
                         << boost::qvm::A<0, 2>(camera_to_hexa) << " "
                         << boost::qvm::A<0, 3>(camera_to_hexa);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<1, 0>(camera_to_hexa) << " "
                         << boost::qvm::A<1, 1>(camera_to_hexa) << " "
                         << boost::qvm::A<1, 2>(camera_to_hexa) << " "
                         << boost::qvm::A<1, 3>(camera_to_hexa);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<2, 0>(camera_to_hexa) << " "
                         << boost::qvm::A<2, 1>(camera_to_hexa) << " "
                         << boost::qvm::A<2, 2>(camera_to_hexa) << " "
                         << boost::qvm::A<2, 3>(camera_to_hexa);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<3, 0>(camera_to_hexa) << " "
                         << boost::qvm::A<3, 1>(camera_to_hexa) << " "
                         << boost::qvm::A<3, 2>(camera_to_hexa) << " "
                         << boost::qvm::A<3, 3>(camera_to_hexa);
      THALAMUS_LOG(info) << "hexa_to_camera";
      THALAMUS_LOG(info) << "  " << boost::qvm::A<0, 0>(hexa_to_camera) << " "
                         << boost::qvm::A<0, 1>(hexa_to_camera) << " "
                         << boost::qvm::A<0, 2>(hexa_to_camera) << " "
                         << boost::qvm::A<0, 3>(hexa_to_camera);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<1, 0>(hexa_to_camera) << " "
                         << boost::qvm::A<1, 1>(hexa_to_camera) << " "
                         << boost::qvm::A<1, 2>(hexa_to_camera) << " "
                         << boost::qvm::A<1, 3>(hexa_to_camera);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<2, 0>(hexa_to_camera) << " "
                         << boost::qvm::A<2, 1>(hexa_to_camera) << " "
                         << boost::qvm::A<2, 2>(hexa_to_camera) << " "
                         << boost::qvm::A<2, 3>(hexa_to_camera);
      THALAMUS_LOG(info) << "  " << boost::qvm::A<3, 0>(hexa_to_camera) << " "
                         << boost::qvm::A<3, 1>(hexa_to_camera) << " "
                         << boost::qvm::A<3, 2>(hexa_to_camera) << " "
                         << boost::qvm::A<3, 3>(hexa_to_camera);
  ...
  boost::asio::awaitable<void> move_objective(float x, float y, float z) {
    ...
    auto new_camera_position =
        boost::qvm::XYZ(camera_position) + boost::qvm::XYZ(camera_offset);

    auto new_hexa_position =
        camera_to_hexa * boost::qvm::XYZ1(new_camera_position);
    co_await move(1000 * boost::qvm::X(new_hexa_position),
                  1000 * boost::qvm::Y(new_hexa_position),
                  1000 * boost::qvm::Z(new_hexa_position));
  }

  boost::asio::serial_port port;

  std::array<std::function<float &(boost::qvm::mat<float, 4, 4> &)>, 16>
      mat_accessors = {
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<0, 0>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<0, 1>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<0, 2>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<0, 3>(m);
          },

          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<1, 0>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<1, 1>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<1, 2>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<1, 3>(m);
          },

          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<2, 0>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<2, 1>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<2, 2>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<2, 3>(m);
          },

          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<3, 0>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<3, 1>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<3, 2>(m);
          },
          [](boost::qvm::mat<float, 4, 4> &m) -> float & {
            return boost::qvm::A<3, 3>(m);
          },
  };

  bool locked = false;

  void on_change(ObservableCollection *source, ObservableCollection::Action,
    ...
    if (key_str == "Running") {
      ...
      get_node_connection =
          graph->get_node_scoped(value_str, [&](std::weak_ptr<Node> weak_node) {
            ...
            auto motion_capture_node =
                node_cast<MotionCaptureNode *>(shared_node.get());
            if (!motion_capture_node) {
              return;
            }
            source_connection =
                shared_node->ready.connect(std::bind(&Impl::on_data, this, _1));
      ...
      hexa_to_camera_state->recap(std::bind(
          &Impl::on_change, this, hexa_to_camera_state.get(), _1, _2, _3));
    }
  }
};

HexascopeNode::HexascopeNode(ObservableDictPtr state,
                             boost::asio::io_context &io_context,
                             NodeGraph *graph)
...
std::string HexascopeNode::type_name() { return "HEXASCOPE"; }

// std::chrono::nanoseconds HexascopeNode::time() const {
//   return 0ns;
// }

bool HexascopeNode::prepare() { return true; }

// For safety we reduce complexity of the hexascope node by requiring a lock
// request before allowing control. Before locking the hexascope will refuse all
// movement commands and after locking it will ignore all observable state
// updates. This reduces the complexity of hexascope control and shields it from
// any errors in the state synchronization logic while still allowing
// configuration through the usual observable bridge API.
boost::json::value
HexascopeNode::process(const boost::json::value &request_value) {
  ...
  if (type == "calibrate") {
    boost::asio::co_spawn(impl->io_context, impl->calibrate(),
                          boost::asio::detached);
    ...
    boost::asio::co_spawn(impl->io_context, impl->align(),
                          boost::asio::detached);
    ...
    boost::asio::co_spawn(impl->io_context, impl->move_objective(x, y, z),
                          boost::asio::detached);
    ...
    boost::asio::co_spawn(impl->io_context, impl->move(x, y, z, x2, y2, z2),
                          boost::asio::detached);
  }
  return response;
}

size_t HexascopeNode::modalities() const {
  return infer_modalities<HexascopeNode>();
}
} // namespace thalamus
