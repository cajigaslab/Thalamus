...
#endif
BOOST_FUSION_ADAPT_STRUCT(thalamus::calculator::signed_,
                          (std::string, sign)(thalamus::calculator::operand,
                                              operand_))

BOOST_FUSION_ADAPT_STRUCT(thalamus::calculator::function_,
                          (std::string, function)(thalamus::calculator::program,
                                                  program_))

BOOST_FUSION_ADAPT_STRUCT(thalamus::calculator::operation,
                          (std::string,
                           operator_)(thalamus::calculator::operand,
                                      operand_)(std::string, operator2_)(
                              thalamus::calculator::operand, operand2_))

BOOST_FUSION_ADAPT_STRUCT(thalamus::calculator::program,
                          (thalamus::calculator::operand,
                           first)(std::list<thalamus::calculator::operation>,
                                  rest))
#ifdef __clang__
#pragma clang diagnostic pop
#endif
...
struct eval {
  ...
  number operator()(nil) const {
    THALAMUS_ASSERT(false, "nil not converable to number");
    return 0;
  }
...
struct parser : qi::grammar<Iterator, program(), ascii::space_type> {
  parser() : parser::base_type(expression1) {
    qi::uint_parser<uint64_t, 10> ulong_;
    ...
    expression1 = boolean >> *(char_("?") >> boolean >> char_(":") >> boolean);

    boolean = expression2 >>
              *((char_("||") >> expression2) | (char_("&&") >> expression2));

    expression2 =
        compare >> *((char_("|") >> compare) | (char_("&") >> compare));

    compare = shift >> *((char_("=") >> shift) | (char_("<>") >> shift) |
                         (char_("<") >> shift) | (char_(">") >> shift) |
                         (char_("<=") >> shift) | (char_(">=") >> shift));

    shift = expression3 >>
            *((char_("<<") >> expression3) | (char_(">>") >> expression3));

    expression3 = term >> *((char_("+") >> term) | (char_("-") >> term));

    term = factor >> *((char_("*") >> factor) | (char_("/") >> factor) |
                       (char_("%") >> factor));

    factor = (char_("ATAN") >> '(' >> expression1 >> ')') |
