...
#include <thalamus/loop_test_node.hpp>
...
struct LoopTestNode::Impl {
  ...
  Impl(ObservableDictPtr state, boost::asio::io_context &io_context,
       NodeGraph *graph, LoopTestNode *outer)
      : state(state), outer(outer), io_context(io_context), graph(graph),
        timer(io_context) {
  ...
  boost::asio::awaitable<void> generate() {
    ...
    while (true) {
      ...
      while (time < current_time - start_time) {
        auto derivative =
            2 * M_PI * frequency * std::cos(2 * M_PI * synth_time);
  ...
  void on_change(ObservableCollection::Action,
    ...
    if (key_str == "Source") {
      ...
      get_node_connection =
          graph->get_node_scoped(value_str, [&](std::weak_ptr<Node> weak) {
...
LoopTestNode::LoopTestNode(ObservableDictPtr state,
                           boost::asio::io_context &io_context,
                           NodeGraph *graph)
...
std::string LoopTestNode::type_name() { return "LOOP_TEST"; }

std::chrono::nanoseconds LoopTestNode::time() const { return impl->time; }

std::span<const double> LoopTestNode::data(int index) const {
  return std::span<const double>(&impl->value, &impl->value + 1);
}

int LoopTestNode::num_channels() const { return 1; }
...
size_t LoopTestNode::modalities() const {
  return infer_modalities<LoopTestNode>();
}
} // namespace thalamus
