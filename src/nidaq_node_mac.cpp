...
struct NidaqNode::Impl {
  ...
  Impl(ObservableDictPtr state, boost::asio::io_context &io_context,
      ...
      : state(state), io_context(io_context), timer(io_context), busy(false),
        analog_buffer_position(0), graph(graph), outer(outer),
        is_running(false) {
    state_connection =
...
std::string NidaqNode::type_name() { return "NIDAQ (MOCK)"; }

std::span<const double> NidaqNode::data(int channel) const {
  return impl->spans.at(channel);
}

int NidaqNode::num_channels() const { return impl->_num_channels; }
...
struct NidaqOutputNode::Impl {
  ...
  Impl(ObservableDictPtr state, boost::asio::io_context &io_context,
       NodeGraph *graph, NidaqOutputNode *outer)
      : state(state), io_context(io_context), timer(io_context), graph(graph),
        outer(outer), running(false) {
  ...
  void on_change(ObservableCollection::Action a,
    ...
    if (key_str == "Source") {
      ...
      graph->get_node(source_str, [&](auto node) {
        ...
        if (!locked_source ||
            node_cast<AnalogNode *>(locked_source.get()) == nullptr) {
          source.reset();
          return;
        }
        source_connection =
...
std::string_view NidaqNode::name(int) const { return "0"; }
} // namespace thalamus
