...
typedef int (*thalamus_ros2_bridge_create_camera_info_publisher_t)(
    const char *topic);
...
struct Ros2Node::Impl {
  ...
  thalamus::map<std::string, boost::signals2::scoped_connection>
      source_connections;
  NodeGraph *graph;
  Ros2Node *outer;
  ...
  void on_mocap_data(Node *, std::shared_ptr<PublishInfo> info) {
    if (info->mocap->has_motion_data()) {
      ...
      auto child_frames = absl::StrSplit(
          info->child_frame, absl::ByAnyChar(", "), absl::SkipEmpty());
  ...
  void on_source_change(std::shared_ptr<PublishInfo> info,
    ...
    if (k_str == "Gaze Topic") {
      ...
      info->gaze_publisher =
          thalamus_ros2_bridge_create_gaze_publisher(v_str.c_str());
      ...
      info->image_publisher =
          thalamus_ros2_bridge_create_image_publisher(v_str.c_str());
  ...
  void on_sources_change(ObservableCollection::Action a,
                         const ObservableCollection::Key &k,
                         const ObservableCollection::Value &v) {
    ...
    if (a == ObservableCollection::Action::Set) {
      ...
      graph->get_node(node_name, [&, node_name, v_dict](auto source) {
        ...
        if (node_cast<DistortionNode *>(locked_source.get()) != nullptr) {
          publish_info->distortion =
              node_cast<DistortionNode *>(locked_source.get());
          publish_info->image = node_cast<ImageNode *>(locked_source.get());
          auto source_connection = locked_source->ready.connect(
              std::bind(&Impl::on_distortion_data, this, _1, publish_info));
          source_connections[node_name] = std::move(source_connection);
        } else if (node_cast<OculomaticNode *>(locked_source.get()) !=
                   nullptr) {
          publish_info->oculomatic =
              node_cast<OculomaticNode *>(locked_source.get());
          publish_info->image = node_cast<ImageNode *>(locked_source.get());
          auto source_connection = locked_source->ready.connect(
              std::bind(&Impl::on_oculomatic_data, this, _1, publish_info));
          source_connections[node_name] = std::move(source_connection);
        } else if (node_cast<ImageNode *>(locked_source.get()) != nullptr) {
          publish_info->image = node_cast<ImageNode *>(locked_source.get());
          auto source_connection = locked_source->ready.connect(
              std::bind(&Impl::on_image_data, this, _1, publish_info));
          source_connections[node_name] = std::move(source_connection);
        }
        ...
        if (node_cast<MotionCaptureNode *>(locked_source.get()) != nullptr) {
          ...
          auto source_connection = locked_source->ready.connect(
              std::bind(&Impl::on_mocap_data, this, _1, publish_info));
          source_connections[node_name] = std::move(source_connection);
        }

        source_mapping_connections[node_name] = v_dict->changed.connect(
            std::bind(&Impl::on_source_change, this, publish_info, _1, _2, _3));
        v_dict->recap(
            std::bind(&Impl::on_source_change, this, publish_info, _1, _2, _3));
      });
  ...
  void on_change(ObservableCollection::Action,
    ...
    if (key_str == "Sources") {
      ...
      sources.clear();
      sources_state_connection = sources_dict->changed.connect(
...
Ros2Node::Ros2Node(ObservableDictPtr state, boost::asio::io_context &io_context,
                   NodeGraph *graph)
    : impl(new Impl(state, io_context, graph, this)) {}

Ros2Node::~Ros2Node() {}

std::string Ros2Node::type_name() { return "ROS2"; }

std::span<const double> Ros2Node::data(int) const {
  return std::span<const double>();
}

int Ros2Node::num_channels() const { return 0; }
...
bool Ros2Node::prepare() {
  ...
  if (library_handle == nullptr) {
    auto error = dlerror();
    THALAMUS_LOG(info) << "Failed to load libthalamus_ros2_bridge.so: "
                       << error;
    return false;
  }
...
void Ros2Node::cleanup() { thalamus_ros2_bridge_stop(); }
} // namespace thalamus
