...
namespace thalamus {
  ...
  public:
    ...
    virtual bool is_short_data() const {
      return false;
    }
  };

  template<typename T>
  class AnalogNodeWrapper {
  private:
  ...
  public:
    ...
    int num_channels() const {
      return underlying->num_channels();
    }
    ...
    std::string_view name(int channel) const {
      return underlying->name(channel);
    }
  };

  template <typename T>
  void visit_node(AnalogNode* node, T callable) {
