...
namespace thalamus {
  ...
  public:
    Impl(ObservableDictPtr state, boost::asio::io_context& io_context, NodeGraph* graph, NormalizeNode* outer)
      : state(state)
      , outer(outer)
      , graph(graph) {
      ...
      if(std::filesystem::exists(std::filesystem::path(".normalize_cache"))) {
        ...
        input.read(reinterpret_cast<char*>(ranges.data()), sizeof(Range)*ranges.size());
        print_ranges();
      }
        ...
        graph->get_node(value_str, [&](auto node) {
          ...
          source_connection = locked->ready.connect([&] (auto n) {
            ...
            if(data.size() < source->num_channels()) {
              ...
              ranges.resize(source->num_channels(), std::make_pair(std::numeric_limits<double>::max(),
            ...
            for(auto i = 0;i < source->num_channels();++i) {
              ...
              for(auto j = 0;j < transformed.size();++j) {
                ...
                x = (x - range.first)/(range.second - range.first + std::numeric_limits<double>::min())*(out_max-out_min) + out_min;
              }
            }
            outer->ready(outer);
          });
  ...
  NormalizeNode::NormalizeNode(ObservableDictPtr state, boost::asio::io_context& io_context, NodeGraph* graph)
    : impl(new Impl(state, io_context, graph, this)) {}

  NormalizeNode::~NormalizeNode() {}

  std::string NormalizeNode::type_name() {
    return "NORMALIZE";
  }
  ...
  int NormalizeNode::num_channels() const {
  ...
  void NormalizeNode::inject(const thalamus::vector<std::span<double const>>& spans, const thalamus::vector<std::chrono::nanoseconds>& sample_intervals, const thalamus::vector<std::string_view>&) {
    THALAMUS_ASSERT(false);
  }
   
  bool NormalizeNode::has_analog_data() const {
    return true;
  }
  ...
  boost::json::value NormalizeNode::process(const boost::json::value& value) {
    ...
    if(text == "Cache") {
      std::ofstream output(".normalize_cache", std::ios::out | std::ios::binary | std::ios::ate);
      output.write(reinterpret_cast<char*>(impl->ranges.data()), sizeof(Range)*impl->ranges.size());
      impl->print_ranges();
    } else if (text == "Reset") {
      impl->ranges.assign(impl->ranges.size(), std::make_pair(std::numeric_limits<double>::max(),
                                                              -std::numeric_limits<double>::max()));
    }
