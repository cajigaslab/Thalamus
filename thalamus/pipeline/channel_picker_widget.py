...
class SourcesModel(QAbstractItemModel):
  ...
  def on_sources_change(self, action, key, value):
    if action == ObservableCollection.Action.SET:
      ...
      self.endInsertRows()
      value.add_observer(lambda *args: self.on_mappings_change(i, *args), functools.partial(isdeleted, self))
      for k, v in enumerate(value):
        self.on_mappings_change(i, ObservableCollection.Action.SET, k, v)
  ...
  def on_mappings_change(self, i, action, key, value):
    print('on_mappings_change', i, action, key, value)
    parent = self.index(i, 0, QModelIndex())
    if action == ObservableCollection.Action.SET:
      self.beginInsertRows(parent, key, key)
      self.endInsertRows()
      value.add_observer(lambda *args: self.on_mapping_change(value, i, key, *args), functools.partial(isdeleted, self))
  ...
  def on_mapping_change(self, self_channel, i, j, action, key, value):
