...
from .util import wait_for, wait_for_hold, RenderOutput, animate, nullcontext, stimulator, get_sound
...
async def run(context: task_context.TaskContextProtocol) -> task_context.TaskResult: #pylint: disable=too-many-statements
  ...
  async def fail_trial():
    ...
    fail_sound.play()  
    await context.log(f'BehavState={State.FAIL.name}') 
  ...
  while True: 
    ...
    show_start_target = False
    context.widget.update()
    await context.log(f'BehavState={State.INTERTRIAL.name}') 
    ...
    blank_space_touched = False
    await context.log(f'BehavState={State.START_ON.name}') 
    state_brightness = toggle_brightness(state_brightness)
  ...
  # state: startacq
  await context.log(f'BehavState={State.START_ACQ.name}')
  ...
  context.widget.update()
  await context.log(f'BehavState={State.TARGS_ON.name}')
  ...
  state_brightness = toggle_brightness(state_brightness)
  context.widget.update()
  await context.log(f'BehavState={State.GO.name}')
  ...
  if quadrant != []:
    ...
    t = np.array([canvas.frameGeometry().width()/2,
          canvas.frameGeometry().height()/2])
    rect = all_target_rects[final_i_selected_target]
    rect_center = rect.center()
    x_rel = abs(gaze_pos.x()-t[0])
    y_rel = abs(gaze_pos.y()-t[1])
    new_x_rel = abs(rect_center.x()-t[0])
    new_y_rel = abs(rect_center.y()-t[1])
    xscale_new = xscale_init*new_x_rel/x_rel
    yscale_new = yscale_init*new_y_rel/y_rel
  ...
  await context.log(f'BehavState={State.SUCCESS.name}')
  state_brightness = toggle_brightness(state_brightness)
  context.widget.update()
