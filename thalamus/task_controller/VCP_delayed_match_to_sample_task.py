"""
Implementation of the delayed match to sample task v1.0 (2025/10/30)
"""
import time
import typing
import numbers
...
task_groups = ["Visual stimulus", "Electrical stimulus"] # Define the possible task groups
shapes = ["square", "circle", "triangle"] # Define the possible shapes
...
class State(enum.Enum):
  ...
  FIXATE1 = enum.auto()
  SAMPLE_PRESENTATION = enum.auto()
  FIXATE2 = enum.auto()
  CHOICE_PRESENTATION = enum.auto()
  ACQUIRE_CHOICE = enum.auto()
  HOLD_CHOICE = enum.auto()
  SUCCESS = enum.auto()
  FAILURE = enum.auto()
  ABORT = enum.auto()
...
def create_widget(task_config: ObservableCollection) -> QWidget:
  ...
  form = Form.build(task_config, ["Name:", "Min:", "Max:"],
    ...
    Form.Uniform('\u23F0 Fixation Duration 1', 'fix1_duration', 200, 400, 'ms'),
    Form.Uniform('\u23F0 Sample Presentation Duration', 'sample_present_dur', 500, 700, 'ms'), 
    Form.Uniform('\u23F0 Fixation Duration 2', 'fix2_duration', 200, 400, 'ms'),
    Form.Uniform('\u23F0 Choice Presention Duration', 'choice_present_dur', 500, 700, 'ms'),
    Form.Uniform('\u23F0 Choice Hold Duration', 'choice_hold_dur', 10, 20, 'ms'),
    Form.Uniform('\u23F0 Decision Timeout', 'decision_timeout', 1000, 2000, 'ms'),
    Form.Uniform('\u23F0 Penalty Delay', 'penalty_delay', 10, 10, 'ms'),
    Form.Constant('\u23F0 Max allowed single blink duration', 'blink_dur_ms', 500, 'ms'),                
    Form.Uniform('\U0001F4A7 Reward per trial', 'reward_pertrial_ms', 200, 300, 'ms'),  
    Form.Constant('\U0001F3A3 Rate of catch trials', 'catch_trial_rate', 0.1),               
    Form.Uniform('\u23F0 Additional catch trial duration', 'catch_duration', 300, 500, 'ms'),  
    Form.Constant('\u25EF Fixation cross - Radius for gaze acceptance', 'accpt_fix_radius_deg', 2, '\u00B0'), # Define the radius in degrees of the area where gaze is accepted as being correct
    Form.Constant('\u25EF Choice - Radius for gaze acceptance', 'accpt_gaze_radius_deg', 2, '\u00B0'), # Define the radius in degrees of the area where gaze is accepted as being correct
    # Form.Choice('Shape', 'shape', list(zip(shapes, shapes))),  # Add the shape attribute
    Form.Constant('\U0001F522 Number of choices', 'num_choices', 2),
    Form.Constant('\u2220 Eccentricity of choice locations', 'choice_eccentricity', 6.5),
  ...
  num_choices_spinbox = form.findChild(QDoubleSpinBox, "num_choices")
  num_choices_spinbox.setRange(2, 8)
  num_choices_spinbox.setSingleStep(1)
...
async def present_sample_func(context, get_gaze, center, accpt_fix_radius_pix, duration1, duration2, monitorsubj_W_pix, monitorsubj_H_pix):
  # Maintain fixation for 'duration' ms during sample presentation (stimulation)
...
async def present_choice_func(context, get_gaze, center, accpt_fix_radius_pix, duration1, duration2, monitorsubj_W_pix, monitorsubj_H_pix):
  # Maintain fixation for 'duration' ms during choice presentation
  # if don't reacquire the target within blink_dur_ms, then ABORT the trial
  success = await wait_for_hold(
      context,
      lambda: abs(QPoint.dotProduct(
...
async def acquire_choice_func(context, trial_type, get_gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, duration1, duration2, duration3, monitorsubj_W_pix, monitorsubj_H_pix):
  ...
  # Maintain fixation at center
    ...
    success = await wait_for_hold(
        ...
        timedelta(seconds=duration3), # fixate for additional duration
        timedelta(seconds=duration2) # allowed single blink duration
    )
  else:
    # Wait for the gaze to move to the choice within the decision timeout
...
async def hold_choice_func(context, trial_type, get_gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, duration1, duration2, monitorsubj_W_pix, monitorsubj_H_pix):
  if trial_type=="catch_trial": 
  # Maintain fixation at center
    success = await wait_for_hold(
        context,
        ...
        timedelta(seconds=duration1), # choice hold duration
        timedelta(seconds=duration2) # allowed single blink duration
    )
  else: 
  # Wait for the gaze to hold on the choice position 
    success = await wait_for_hold(
        context,
        lambda: abs(QPoint.dotProduct(
            gaze_valid(get_gaze(), monitorsubj_W_pix, monitorsubj_H_pix) - targetpos_pix,
...
# Center fixation during sample presentation for 'sample_present_dur' ms
async def handle_present_sample(
    context,
    task_group,
    get_gaze,
    center,
    ...
    global state
    state = State.SAMPLE_PRESENTATION
    await context.log('BehavState=SAMPLE_PRESENTATION_post-drawing_PHOTODIODE-SQUARE')
    print(state)
    widget.update()
    success = await present_sample_func(context, get_gaze, center, accpt_fix_radius_pix, sample_present_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix)
    return success

# Center fixation after sample presentation for 'fix2_duration' ms
...
# Center fixation during choice presentation for 'choice_present_dur' ms
async def handle_present_choice(
    context,
    task_group,
    get_gaze,
    center,
    ...
    global state
    state = State.CHOICE_PRESENTATION
    await context.log('BehavState=CHOICE_PRESENTATION_post-drawing_PHOTODIODE-SQUARE')
    print(state)
    widget.update()
    success = await present_choice_func(context, get_gaze, center, accpt_fix_radius_pix, choice_present_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix)
    return success

# Saccade to choice within 'decision_timeout' ms (saccade trial) OR 
# Stay fixated for 'decision_timeout' ms (catch trial)
async def handle_acquire_choice(
    context,
    trial_type,
    get_gaze,
    center,
    ...
    global state
    state = State.ACQUIRE_CHOICE
    await context.log('BehavState=ACQUIRE_CHOICE_start')
    print(state)
    widget.update()
    success = await acquire_choice_func(context, trial_type, get_gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, decision_timeout, blink_dur_ms, catch_duration, monitorsubj_W_pix, monitorsubj_H_pix)
    temp_gaze = gaze_valid(get_gaze(), monitorsubj_W_pix, monitorsubj_H_pix)
...
# Maintain fixation at target for 'choice_hold_dur' ms (saccade trial) OR
# Stay fixated for 'choice_hold_dur' ms (catch trial)
async def handle_hold_choice(
    context,
    trial_type,
    get_gaze,
    center,
    ...
    global state
    state = State.HOLD_CHOICE
    await context.log('BehavState=HOLD_CHOICE_start')
    print(state)
    widget.update()
    success = await hold_choice_func(context, trial_type, get_gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, choice_hold_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix)
    temp_gaze = gaze_valid(get_gaze(), monitorsubj_W_pix, monitorsubj_H_pix)
...
async def run(context: TaskContextProtocol) -> TaskResult: #pylint: disable=too-many-statements
  """Main entry point for the delayed match to sample task."""
  global converter, center, center_f, rand_pos_i, task_group, \
  ...
  num_choices, choice_eccentricity, sample_shape, choice_shapes, choice_pos, choice_idx
  ...
  def get_valid_angles(step_deg, sector1_min, sector1_max):
    all_angles = [i for i in np.linspace(0, 360, int(step_deg)+1)]
    valid_angles = []
    for angle in all_angles:
        if (angle_in_sector(angle, sector1_min, sector1_max)):
            valid_angles.append(angle)
    valid_angles = sorted(set(valid_angles))[:-1]
    return sorted(set(valid_angles))  # remove duplicates if overlapping
  
  def setup_sample_and_choices(num_choices: int) -> int:
    """
    For this trial:
      - Pick a random sample shape from `shapes`.
      - Sample `num_choices` positions from `rand_pos`.
      - Build `choice_shapes` and `choice_pos` (same length).
      - Exactly ONE of those positions is assigned the sample shape.
    Returns:
      correct_idx  (int): index in [0, num_choices) where the sample is.
    """
    global sample_shape, choice_shapes, choice_pos, rand_pos

    # 1) sample shape
    sample_shape = random.choice(shapes)

    # 2) choose positions from rand_pos
    choice_pos = list(rand_pos)

    # 3) assign shapes – one of them must be the sample
    choice_shapes = [None] * int(num_choices)

    # randomly choose which index will be correct (the sample)
    correct_idx = random.randrange(num_choices)
    choice_shapes[correct_idx] = sample_shape

    # pool of distractors 
    distractors = [s for s in shapes if s != sample_shape]
    if not distractors:
        distractors = [sample_shape]  

    for i in range(num_choices):
        if choice_shapes[i] is None:
            choice_shapes[i] = random.choice(distractors)

    return sample_shape, correct_idx, choice_pos


  def draw_shape(painter: QPainter, shape_name: str) -> None:
  ...
  def draw_sample(painter: QPainter) -> None:
    """
    Draw the sample shape at the center of the subject screen.
    """
    if sample_shape is None:
        print("No sample shape to draw.")
        return

    painter.save()
    # painter.translate(center.x(), center.y())
    draw_shape(painter, sample_shape)
    # painter.rotate(orientation_targ_ran) 
  ...
  def draw_choice(painter: QPainter) -> None:
    """
    Draw all choice shapes (sample + distractors) at their positions.
    """
    if not choice_shapes or not choice_pos:
        return

    for shp, pos in zip(choice_shapes, choice_pos):
        # x, y = pos
        painter.save()
        painter.translate(pos[0]-center.x(), pos[1]-center.y())
        draw_shape(painter, shp)
        painter.restore()
  ...
  if converter is None: 
    ...
    center = QPoint(int(converter.screen_pixels.width)//2, int(converter.screen_pixels.height)//2)
    center_f = QPointF(float(converter.screen_pixels.width)/2.0, float(converter.screen_pixels.height)/2.0)
    ...
    num_choices = int(config['num_choices'])
    choice_eccentricity = config['choice_eccentricity']  # keep in degrees
    rand_pos = []  # global list of saccade/choice target positions
    for ang_deg in get_valid_angles(num_choices,0,360):
        ang_rad = np.deg2rad(ang_deg)
        x_deg = choice_eccentricity * np.cos(ang_rad)
        y_deg = choice_eccentricity * np.sin(ang_rad)

        # converter.deg_to_pixel_abs gives absolute pixel coords for (x_deg, y_deg) relative to center
        x_pix, y_pix = converter.deg_to_pixel_abs((x_deg, y_deg))
        rand_pos.append((x_pix, y_pix))
  ...
  #   setup_sample_and_choices(targetpos_pix_f, num_choices)
  ...
  if trial_type == "saccade_trial":
    # build shapes + positions for this trial
    sample_shape, correct_idx, choice_pos = setup_sample_and_choices(num_choices)

    # saccade target = the sample's position
    targetpos_pix = QPoint(int(choice_pos[correct_idx][0]),int(choice_pos[correct_idx][1]))
    targetpos_pix_f = QPointF(int(targetpos_pix.x()), int(targetpos_pix.y()))
    ...
    targetpos_pix = QPoint(center.x(), center.y())
    targetpos_pix_f = QPointF(center.x(), center.y())

    # no shapes on catch trials
    sample_shape = None
    choice_shapes = []
    choice_pos = []


  # Build fixation cross path
  ...
  square_vertices = [converter.deg_to_pixel_abs(p) for p in square_deg]
  square = QPainterPath()
  square.moveTo(square_vertices[0][0], square_vertices[0][1])
  square.lineTo(square_vertices[1][0], square_vertices[1][1])
  square.lineTo(square_vertices[2][0], square_vertices[2][1])
  square.lineTo(square_vertices[3][0], square_vertices[3][1])
  square.closeSubpath()
  ...
  triangle_vertices = [converter.deg_to_pixel_abs(p) for p in triangle_deg]
  triangle = QPainterPath()
  triangle.moveTo(triangle_vertices[0][0], triangle_vertices[0][1])
  triangle.lineTo(triangle_vertices[1][0], triangle_vertices[1][1])
  triangle.lineTo(triangle_vertices[2][0], triangle_vertices[2][1])
  triangle.closeSubpath()

  # Build circle path
  x0, y0 = square_vertices[0]
  x2, y2 = square_vertices[2]   
  w = x2 - x0
  h = y2 - y0
  circle = QPainterPath()
  circle.moveTo(0,0)
  circle.addEllipse(x0, y0, w, h)
...
# opposite corner
  ...
  num_choices = config['num_choices']
  choice_eccentricity = converter.deg_to_pixel_rel(config['choice_eccentricity'])
  ...
  sample_present_dur = context.get_value('sample_present_dur') / 1000
  fix2_duration = context.get_value('fix2_duration') / 1000
  choice_present_dur = context.get_value('choice_present_dur') / 1000
  choice_hold_dur = context.get_value('choice_hold_dur') / 1000
  ...
  def renderer(painter: QPainter):
    ...
    # Draw the fixation cross, sample and choices based on the current state
    if state in (State.ACQUIRE_FIXATION, State.FIXATE1):
      ...
      if paint_all_targets:
        ...
        for angle in np.linspace(0, 360, num_choices):
    ...
    elif state == State.SAMPLE_PRESENTATION:
      # await context.log('BehavState=SAMPLE_PRESENTATION_start') # saving any variables / data from code
      pen = painter.pen()
      pen.setWidth(2)
      pen.setColor(QColor(128, 0, 128))
      painter.setPen(pen)
      painter.drawPath(cross)
      # if task_group == "Visual stimulus":
      pen = painter.pen()
      pen.setWidth(2)
    ...
    elif state == State.CHOICE_PRESENTATION:
      # await context.log('BehavState=CHOICE_PRESENTATION_start') # saving any variables / data from code
      pen = painter.pen()
      pen.setWidth(2)
      pen.setColor(QColor(128, 0, 128))
      painter.setPen(pen)
    ...
    elif state in (State.HOLD_CHOICE, State.ACQUIRE_CHOICE):
      if trial_type == "saccade_trial":
        pen = painter.pen()
        pen.setWidth(2)
        pen.setColor(QColor(255, 255, 255))
    ...
    with painter.masked(RenderOutput.OPERATOR): # using a context manager to temporarily change the drawing behavior of the painter object
      ...
      drawText(painter, f"Saccade_trial = {trial_saccade_success_count} / {trial_saccade_count}", QPoint(0, 100), background_color_qt) # Draw the text message
      drawText(painter, f"Catch_trial = {trial_catch_success_count} / {trial_catch_count}", QPoint(0, 130), background_color_qt) # Draw the text message
      drawText(painter, f"Total_reward = {round(reward_total_released_ms)} ms", QPoint(0, 160), background_color_qt) # Draw the text message
  ...
  # Handles State.SAMPLE_PRESENTATION
  success = await handle_present_sample(context, task_group, lambda: gaze, center, accpt_fix_radius_pix, sample_present_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix, widget)
  ...
  await handle_fixate2(context, task_group, lambda: gaze, center, accpt_fix_radius_pix, fix2_duration, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix, widget)
  # state = State.FIXATE2
  ...
  # Handles State.SAMPLE_PRESENTATION
  success = await handle_present_choice(context, task_group, lambda: gaze, center, accpt_fix_radius_pix, choice_present_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix, widget)
  ...
  # Handles State.ACQUIRE_CHOICE
  success = await handle_acquire_choice(context, trial_type, lambda: gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, decision_timeout, blink_dur_ms, catch_duration, monitorsubj_W_pix, monitorsubj_H_pix, widget, converter)
  ...
  success = await handle_hold_choice(context, trial_type, lambda: gaze, center, targetpos_pix, accpt_fix_radius_pix, accpt_gaze_radius_pix, choice_hold_dur, blink_dur_ms, monitorsubj_W_pix, monitorsubj_H_pix, widget, converter)
