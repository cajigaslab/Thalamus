...
Config = typing.NamedTuple('Config', [
  ...
  ('targ1_off_delay',datetime.timedelta),
  ('targ2_gap', datetime.timedelta),
  ...
  ('is_choice', bool),
  ('targets_are_second_targets', bool)
])
...
class TargetWidget(QWidget):
  ...
  def __init__(self, config: ObservableCollection) -> None:
    ...
    fixed_form = Form.build(config, ['Name:', 'Value:'],
      ...
      Form.Bool('Is Second Target', 'is_targ2', False),
...
def create_widget(task_config: ObservableCollection) -> QWidget:
  ...
  form = Form.build(task_config, ["Name:", "Min:", "Max:"],
    ...
    Form.Uniform('Targ1 Off Delay', 'targ1_off_delay', 0.2, 0.2, 's'),
    Form.Uniform('Targ2 Gap', 'targ2_gap', 0.05, 0.05, 's'),
    ...
    Form.Bool('Targets Are Second Targets', 'targets_are_second_targets', False)
  )
...
def make_relative_target_rect(context, i_targ2, origin_target_rect, dpi):
  x_ecc, y_ecc = pol_to_cart2d(
...
async def run(context: task_context.TaskContextProtocol) -> task_context.TaskResult: #pylint: disable=too-many-statements
  ...
  config = Config(
    ...
    datetime.timedelta(seconds=context.get_value('targ1_off_delay', RANDOM_DEFAULT)),    
    datetime.timedelta(seconds=context.get_value('targ2_gap', RANDOM_DEFAULT)),    
    ...
    context.get_value('targets_are_second_targets')
  )
  ...
  if config.targets_are_second_targets:
    ...
    targ2_rect = make_relative_target_rect(context, 
      i_presented_targ2, 
  ...
  def touch_handler(cursor: QPoint) -> None:
    ...
    if config.is_choice:
      ...
      if not config.targets_are_second_targets:
        targ2_acquired = False # by definition
      else:
        targ2_acquired = distance(all_target_rects[i_presented_targ2], cursor) < all_target_windows[i_presented_targ2]    
    else:
      ...
      targ2_acquired = distance(all_target_rects[i_presented_targ2], cursor) < all_target_windows[i_presented_targ2]
      if targ2_acquired:
        i_selected_target = i_presented_targ2
        last_selected_target = i_selected_target
      else:
        i_selected_target = None
  ...
  def renderer(painter: QPainter) -> None:
    ...
    if config.is_choice:
      ...
      if show_targ2:
        stl_mesh = all_target_stls[i_presented_targ2]
        if stl_mesh:
          painter.render_stl(stl_mesh)
        else:
          painter.fillRect(all_target_rects[i_presented_targ2], all_target_colors[i_presented_targ2])
  ...
  if config.is_choice:
    ...
    behav_result['presented_target_ids'] = [i_presented_targ, int(i_presented_targ2)]

  #behav_result['presented_targ2_id'] = int()
  ...
  async def jump_targ():
    ...
    nonlocal show_presented_target
    nonlocal show_targ2
    await context.sleep(config.targ1_off_delay)    
    show_presented_target = False
    state_brightness = toggle_brightness(state_brightness)
    await context.servicer.publish_state(task_controller_pb2.BehavState(state='targ1_off'))
    context.widget.update()

    await context.sleep(config.targ2_gap)
    show_targ2 = True
    state_brightness = toggle_brightness(state_brightness)
  ...
  acquired = await wait_for(context, lambda: targ2_acquired, config.reach_timeout)
  if not acquired:
    await fail_trial()
    await context.sleep(config.fail_timeout)
    return task_context.TaskResult(False)
  ...
  success = await wait_for_hold(context, lambda: targ2_acquired, config.hold_timeout, config.blink_timeout)
  if not success:
    await fail_trial()
    await context.sleep(config.fail_timeout)
    return task_context.TaskResult(False)
