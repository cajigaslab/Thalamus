...
from .util import wait_for, wait_for_hold, wait_for_dual_hold, RenderOutput, animate, nullcontext, stimulator, get_sound
...
def create_widget(task_config: ObservableCollection) -> QWidget:
  ...
  form = Form.build(task_config, ["Name:", "Min:", "Max:"],
    ...
    Form.Uniform('Eye Blink', 'eye_blink', 0.25, 0.25, 's'),
    Form.Uniform('Fail Interval', 'fail_timeout', 2, 2, 's'),
    Form.Uniform('Success Interval', 'success_timeout', 0.5, 1, 's'),
...
async def next_state(context, new_state, stim_phase, stim_start, intan_cfg, pulse_width, pulse_count, pulse_period):
  ...
  # state_message.state = new_state.name
  ...
  def renderer(painter: QPainter) -> None:
    ...
    with painter.masked(RenderOutput.OPERATOR):
      ...
      for rect in all_target_rects:
        #window = all_target_windows[rect]
        path.addEllipse(QPointF(rect.center()), window,window)#all_target_windows[rect], all_target_windows[rect]) # IG: need to confirm this works. If so, all tasks should use this logic.
