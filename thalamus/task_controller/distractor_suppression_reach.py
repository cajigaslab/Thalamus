...
#type: ignore
"""
Implementation of the distractor suppression task
"""
import time
import math
import typing
...
class TargetWidget(PyQt5.QtWidgets.QWidget):
  ...
  def __init__(self, config: ObservableCollection) -> None:
    ...
    fixed_form = Form.build(config, ['Name:', 'Value:'],
      ...
      Form.Bool('Is Distractor', 'is_distractor', False),
...
def create_widget(task_config: ObservableCollection) -> PyQt5.QtWidgets.QWidget:
  ...
  form = Form.build(task_config, ["Name:", "Min:", "Max:"],
    ...
    Form.Bool('Targets Are Distractors', 'targets_are_distractors', False)
  )
...
def get_target_rectangles(context, dpi):  
  ...
  ntargets = len(context.task_config['targets']) # all targets, including fixation and distractors
    
  all_target_rects = []  
  for itarg in range(ntargets): # looping through all targets, including fixation

    if not context.get_target_value(itarg, 'is_distractor'):
...
def make_relative_distractor_rect(context, i_distractor, origin_target_rect, dpi):
  x_ecc, y_ecc = pol_to_cart2d(
    context.get_target_value(i_distractor, 'radius'),
    context.get_target_value(i_distractor, 'angle'))

  targ_width_px = ecc_to_px(context.get_target_value(i_distractor, 'width'), dpi)
  targ_height_px = ecc_to_px(context.get_target_value(i_distractor, 'height'), dpi)
...
async def run(context: task_context.TaskContextProtocol) -> task_context.TaskResult: #pylint: disable=too-many-statements
  ...
  config = Config(
    ...
    context.get_value('targets_are_distractors')
  )
  ...
  i_targs = [i for i, x in enumerate(context.task_config['targets']) if not x['is_distractor']] # including fixation, but not distractors
  i_distractors = [i for i, x in enumerate(context.task_config['targets']) if x['is_distractor']] # only distractors
  
  distractors = [context.task_config['targets'][i] for i in i_distractors]

  ntargets = len(context.task_config['targets']) # including fixation and distractors
  ...
  all_target_rects = get_target_rectangles(context, dpi) # fixation, all targets, all distractors (None entry for distractors for now)
  all_target_rects_no_distractors = [all_target_rects[i] for i in i_targs]
  ...
  if config.targets_are_distractors:
    i_presented_distractor = np.random.choice(np.setdiff1d(i_periph_targs, i_presented_targ))
    distractor_rect = all_target_rects[i_presented_distractor]
  else:
    distractor_stl = None
    i_presented_distractor = np.random.choice(i_distractors)    
    distractor_rect = make_relative_distractor_rect(context, 
      i_presented_distractor, 
      all_target_rects[i_presented_targ], 
      dpi)
  distractor_stl = all_target_stls[i_presented_distractor]
  distractor_color = all_target_colors[i_presented_distractor]
  all_target_rects[i_presented_distractor] = distractor_rect
  ...
  def touch_handler(cursor: PyQt5.QtCore.QPoint) -> None:
    ...
    if config.is_choice:
      ...
      in_periph_windows = [distance(all_target_rects[i].center(), cursor) < all_target_windows[i]
                                    for i in range(ntargets) if i != i_start_targ and i != i_presented_distractor]
    ...
    if config.is_choice:
      if show_presented_target:
        for i, value in enumerate(zip(all_target_rects, all_target_colors, all_target_stls)):
          if i == i_start_targ or i in i_distractors:
            continue
      ...
      if show_distractor_target:                
        if distractor_stl:
          painter.render_stl(distractor_stl)
        else:
          painter.fillRect(distractor_rect, distractor_color)

    with painter.masked(RenderOutput.OPERATOR):
      path = PyQt5.QtGui.QPainterPath()

      for rect in all_target_rects_no_distractors:
  ...
  async def fail_trial():    
    ...
    nonlocal show_distractor_target
  ...
  start_targ_released = await wait_for(context, lambda: not start_target_acquired, config.reach_timeout)
  ...
  await asyncio.gather(
                    present_distractor_after(config.targ2_delay), 
                    wait_and_process_target_acq())
  if failed:
    return task_context.TaskResult(False)

  final_i_selected_target = last_selected_target
  behav_result['selected_target_id'] = final_i_selected_target
  behav_result['presented_distractor_id'] = int(i_presented_distractor)
