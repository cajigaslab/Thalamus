...
from .thalamus_pb2 import StimDeclaration, Pulse
...
async def run(context: task_context.TaskContextProtocol) -> task_context.TaskResult: #pylint: disable=too-many-statements  
  ...
  stim = StimDeclaration(
    pulse = Pulse(amplitude=5,duration_ns=1_000_000*on_time_ms),
    id=0
  )

  await context.arm_stim('NIDAQ_OUT', stim)
  await context.start_stim('NIDAQ_OUT', 0)
