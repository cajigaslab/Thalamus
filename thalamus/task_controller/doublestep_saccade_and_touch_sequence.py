...
#type: ignore
"""
Implementation of doublestep_saccade_and_touch_sequence task. This uses the logic of doublestep_saccade_and_touch_fast to address display latency.
"""
import time
import math
import typing
...
from .util import wait_for, wait_for_hold, wait_for_dual_hold, RenderOutput, animate, get_sound
...
async def run(context: task_context.TaskContextProtocol) -> task_context.TaskResult: #pylint: disable=too-many-statements
  ...
  def gaze_handler(cursor: QPoint) -> None:
    ...
    targ2_gazed = bool(targ2_distances)
    selected_targ2 = targ2_distances[0] if targ2_gazed else None
  ...
  context.widget.update()
  await context.log(f'BehavState=start_gaze_on')
  acquired = await wait_for(context, lambda: presented_targ_gazed, config.start_timeout)
  if not acquired:
    await fail_trial()
    await context.sleep(config.fail_timeout)
    print("here3")
  ...
  success = await wait_for_dual_hold(context, config.baseline_timeout, 
    lambda: presented_targ_gazed, lambda: start_target_touched, 
  ...
  acquired = await wait_for(context, lambda: start_target_touched and start_target_gazed or blank_space_touched, timeout)  
  ...
  if (not acquired and elapsed_time>=config.saccade_timeout) or blank_space_touched:
    ...
    acquired = await wait_for(context, lambda: start_target_touched and start_target_gazed or blank_space_touched, remaining_time)  
  ...
  blank_space_touched = False
  acquired = await wait_for(context, lambda: targ2_gazed and start_target_touched or blank_space_touched, config.saccade_timeout)
  ...
  success = await wait_for_dual_hold(context, config.hold_interval, 
    lambda: start_target_touched, lambda: targ2_gazed, 
    config.hand_blink, config.eye_blink)
  if not success:
    await fail_trial()
    await context.sleep(config.fail_timeout)
