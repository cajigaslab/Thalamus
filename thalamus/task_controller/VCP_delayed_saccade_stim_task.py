"""
Integration of intan and thalamus to deliver stimulation during a delayed saccade task
"""
...
def create_widget(task_config: ObservableCollection) -> QWidget:
  ...
  Form.Constant('\u23F0 Max allowed single blink duration', 'blink_dur_ms', 10, 'ms'),                
  Form.Uniform('\U0001F4A7 Reward per trial', 'reward_per_trial_ms', 100, 150, 'ms'),
  Form.Constant('\u26A1 Rate of stim trials', 'stim_trial_rate', 0.8),   
  ...
  Form.Constant('\u2194 Target width step', 'width_targ_deg_step', 0.1, '\u00B0'),
  Form.Uniform('\u2195 Target height', 'height_targ_deg', 1, 2, '\u00B0'),
  Form.Constant('\u2195 Target height step', 'height_targ_deg_step', 0.1, '\u00B0'),
  ...
  Form.Constant('\u26A1 Stimulation amplitude', 'stim_amplitude', 1000, 'uA'),
  Form.Constant('\u26A1 Stimulation pulse width', 'stim_pulsewidth', 50, 'us'),
  Form.Constant('\u26A1 Stimulation inter phase interval', 'stim_interphase', 50, 'us'),
  Form.Constant('\u26A1 Stimulation frequency', 'stim_frequency', 100, 'Hz'),
  Form.Constant('\u26A1 Stimulation channel', 'stim_channel', 0, ''),
  Form.Constant('\u26A1 Stimulation pulse train duration', 'stim_duration', 0.1,'s'),
  Form.Constant('\u26A1 Intan instance number', 'intan_num', 1, '')
  )
  ...
  width_targ_deg_step_spinbox = form.findChild(QDoubleSpinBox, "width_targ_deg_step")
  width_targ_deg_step_spinbox.setRange(.1, 1.0)
  width_targ_deg_step_spinbox.setSingleStep(.1)
  height_targ_deg_step_spinbox = form.findChild(QDoubleSpinBox, "height_targ_deg_step")
  height_targ_deg_step_spinbox.setRange(.1, 1.0)
  height_targ_deg_step_spinbox.setSingleStep(.1)
...
async def run(context: TaskContextProtocol) -> TaskResult: #pylint: disable=too-many-statements
  ...
  global converter, center, center_f, circle_radii, rand_pos_i, task_group, \
    trial_num, trial_saccade_count, trial_saccade_success_count, trial_catch_count, trial_catch_success_count, trial_stim_count, trial_stim_success_count,\
    drawn_objects, rand_pos, reward_total_released_ms, \
    ...
    stim_amplitude, stim_pulsewidth, stim_interphase, stim_frequency, stim_channel, stim_duration, stim_npulses, stim_node, trigger
  ...
  if converter is None: 
    ...
    # Define the target location parameters
    ...
    stim_amplitude = config['stim_amplitude'] # in uA
    stim_pulsewidth = config['stim_pulsewidth'] # in us
    stim_interphase = config['stim_interphase'] # in us
    stim_frequency = config['stim_frequency'] # in Hz
    stim_channel = int(config['stim_channel']) # channel number (1-8)
    stim_duration = config['stim_duration'] # in s
    stim_npulses = int(stim_duration*stim_frequency) # calculate number of pulses based on frequency and duration
    stim_node = f"intan{int(config['intan_num'])}"
    print(stim_node)


    # Update stim parameters in intan
    trigger = "DigitalIn02"
    # trigger = "KeyPressF1"
    # trigger = "keypressf" + str(config['stim_channel'])
    for node in ["intan1", "intan2"]:
      await context.node_request(node, "set digital-in-01.enabled True;\n") # fiducial
      # await context.sleep(timedelta(seconds=0.05))
      await context.node_request(node, "set digital-in-02.enabled True;\n") # stimulation trigger
      await context.node_request(node, "set analog-in-1.enabled True;\n") # photodiode
      # await context.node_request(node,"set runmode stop;\n")
      for port in ["b"]:
      # for port in ["a", "b", "c", "d"]:
        for ch in range(2):
          await context.node_request(node, f"set {port}-{ch:03d}.stimenabled True;\n")
          await context.node_request(node, f"set {port}-{ch:03d}.source {trigger};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.shape BiphasicWithInterphaseDelay;\n")
          await context.node_request(node, f"set {port}-{ch:03d}.polarity NegativeFirst;\n")
          await context.node_request(node, f"set {port}-{ch:03d}.pulseortrain PulseTrain;\n")
          await context.node_request(node, f"set {port}-{ch:03d}.FirstPhaseAmplitudeMicroAmps {stim_amplitude};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.SecondPhaseAmplitudeMicroAmps {stim_amplitude};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.FirstPhaseDurationMicroseconds {stim_pulsewidth};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.SecondPhaseDurationMicroseconds {stim_pulsewidth};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.InterphaseDelayMicroseconds {stim_interphase};\n")
          await context.node_request(node, f"set {port}-{ch:03d}.PulseTrainPeriodMicroseconds {1000000 / stim_frequency};\n") # calculate based on number of pulses and duration of each pulse
          await context.node_request(node, f"set {port}-{ch:03d}.NumberOfStimPulses {stim_npulses};\n")
          # await context.node_request(node, f"set {port}-{ch:03d}.stimenabled False;\n")
      # await context.sleep(timedelta(seconds=0.1))
      await context.node_request(node,"execute uploadstimparameters;\n")
      await context.sleep(timedelta(seconds=10)) # wait to ensure parameters are uploaded before starting the task
      # await context.node_request(node,"set runmode run;\n")
      await context.node_request(node,"set runmode record;\n")


    await context.node_request(stim_node, f"set {port}-{stim_channel:03d}.stimenabled True;\n")
  ...
  # Update stimulation parameters if user changes
  if stim_amplitude != config['stim_amplitude'] or \
      stim_pulsewidth != config['stim_pulsewidth'] or \
      stim_interphase != config['stim_interphase'] or \
      stim_channel != int(config['stim_channel']) or \
      stim_frequency != config['stim_frequency'] or \
      stim_duration != config['stim_duration'] or \
      stim_node != f"intan{int(config['intan_num'])}":
      print("xxxxxxxxxxxxxxxxxxxxxxxxxCHANGE FLAGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
      print("xxxxxxxxxxxxxxxxxxxxxxxxxCHANGE FLAGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
      print('detected change!!')
      print(f"stim_amplitude={stim_amplitude}\n")
      print(f"stim_pulsewidth={stim_pulsewidth}\n")
      print(f"stim_interphase={stim_interphase}\n")
      print(f"stim_frequency={stim_frequency}\n")
      print(f"stim_duration={stim_duration}")

      stim_amplitude = config['stim_amplitude']
      stim_pulsewidth = config['stim_pulsewidth']
      stim_interphase = config['stim_interphase']
      stim_channel = int(config['stim_channel'])
      stim_frequency = config['stim_frequency']
      stim_duration = config['stim_duration']
      stim_npulses = int(stim_duration*stim_frequency)
      stim_node = f"intan{int(config['intan_num'])}"

      print("xxxxxxxxxxxxxxxxxxxxxxxxxRECORDING STOP FLAGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
      print("xxxxxxxxxxxxxxxxxxxxxxxxxRECORDING STOP FLAGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
      ...
      for node in ["intan1", "intan2"]:
        ...
        # await context.sleep(timedelta(seconds=0.1))
        await context.node_request(node,"execute uploadstimparameters;\n")
        await context.sleep(timedelta(seconds=3.2)) # wait to ensure parameters are uploaded before starting the task
        # await context.node_request(node,"set runmode run;\n")
        await context.node_request(node,"set runmode record;\n")
  ...
  elif random.random() < config['stim_trial_rate']:
    trial_type = "stim_trial"
  else:
  ...
  if trial_type == "saccade_trial": # For saccade trials, the target position is randomly selected from the list of random positions
    ...
    rand_pos_i += 1
  elif trial_type == "stim_trial": # For stim trials, the target position is randomly selected from the list of random positions but is stored separately in the trial summary data for later analysis
    targetpos_pix = QPoint(int(rand_pos[rand_pos_i][0]), int(rand_pos[rand_pos_i][1]))
    context.trial_summary_data.used_values['targetposX_pix'] = targetpos_pix.x()
    context.trial_summary_data.used_values['targetposY_pix'] = targetpos_pix.y()
    targetpos_pix_f = QPointF(int(rand_pos[rand_pos_i][0]), int(rand_pos[rand_pos_i][1]))
  ...
  width_targ_pix = converter.deg_to_pixel_rel(pick_random_value(config['width_targ_deg']['min'], config['width_targ_deg']['max'], config['width_targ_deg_step']))
  context.trial_summary_data.used_values['width_targ_pix'] = width_targ_pix 
  if is_height_locked:
    height_targ_pix = width_targ_pix
  else:
    height_targ_pix = converter.deg_to_pixel_rel(pick_random_value(config['height_targ_deg']['min'], config['height_targ_deg']['max'], config['height_targ_deg_step']))
  ...
  def renderer(painter: QPainter):
    ...
    elif state == (State.HOLD_TARGET, State.ACQUIRE_TARGET):
      if task_group == "Saccade - No Delay": # if we want to move gaze to the target without it disappearing
        pen = painter.pen()
        pen.setWidth(2)
        pen.setColor(QColor(255, 0, 0))
    ...
    with painter.masked(RenderOutput.OPERATOR): # using a context manager to temporarily change the drawing behavior of the painter object
      ...
      painter.fillRect(QRect(0, 0, 465, 260), QColor(255, 255, 255, 255)) # background white rectangle in the OView to see text

      if trial_type == 'saccade_trial' or trial_type == 'stim_trial':
          path = QPainterPath()
      ...
      # drawText(painter, f"Target_presentation = {target_present_dur*1000:.0f} ms", QPoint(0, 100), background_color_qt) # Draw the text message
      drawText(painter, f"Stim_trial = {trial_stim_success_count} / {trial_stim_count}", QPoint(0, 130), background_color_qt) # Draw the text message
      drawText(painter, f"Saccade_trial = {trial_saccade_success_count} / {trial_saccade_count}", QPoint(0, 160), background_color_qt) # Draw the text message
      drawText(painter, f"Catch_trial = {trial_catch_success_count} / {trial_catch_count}", QPoint(0, 190), background_color_qt) # Draw the text message
      ...
      drawText(painter, f"Gaze (pix): x = {temp_gaze.x()}, y = {temp_gaze.y()}", QPoint(0, 220), background_color_qt)
  ...
  if trial_stim_count == 0: # to avoid division by 0 error
    stim_trial_success_rate = 0
  else:
    stim_trial_success_rate = trial_stim_success_count/trial_stim_count*100
  ...
  create_task_with_exc_handling(context.inject_analog('performance', AnalogResponse(
    data = [float(trial_num), stim_trial_success_rate, saccade_trial_success_rate, catch_success_rate],
    spans=[Span(begin=0, end=1, name='Trial Number'), Span(begin=1, end=2, name='Stim trial success rate (%)'), \
            Span(begin=2, end=3, name='Saccade trial success rate (%)'), Span(begin=3, end=4, name='Catch trial success rate (%)')], sample_intervals=[0, 0, 0, 0]
  )))
  ...
  elif trial_type == "saccade_trial": # With Delay or No Delay
    trial_saccade_count += 1 # Increment the saccade trial counter
  else: # trial_type == "stim_trial" With Delay or No Delay
    trial_stim_count += 1 # Increment the stim trial counter
  ...
  if trial_type == "stim_trial":
    # triggerpulse command in intan
    stim_port = "B"
    # stim_ch = 0
    trigger_dur_ms = [100, 100]

    # await context.node_request(stim_node,"set runmode stop;")
    # await context.sleep(timedelta(seconds=0.01))
    # await context.node_request(stim_node, f"set {stim_port}-{stim_channel:03d}.stimenabled True;")
 
    # NIDAQ_out stimulation trigger
    create_task_with_exc_handling(context.inject_analog('stim_in', AnalogResponse(
    data=[5,0], # 5 = HIGH, 0 = LOW voltages
    spans=[Span(begin=0,end=2,name='Stim')], 
    sample_intervals=[1_000_000*dur for dur in trigger_dur_ms]) # multiplyin by 1_000_000 will give us nanoseconds (ns)
    ))

    # TCP command stimulation trigger
    # await context.node_request(stim_node,"execute manualstimtriggerpulse f1;")
    # await context.sleep(timedelta(seconds=0.1))

    # await context.node_request(stim_node, f"set {stim_port}-{stim_ch:03d}.stimenabled False;")
    # await context.sleep(timedelta(seconds=0.01))
    # await context.node_request(stim_node,"set runmode run;")
    await context.log("stim") # saving any variables / data from code
  ...
  if task_group == "Saccade - With Delay":
    if not success:
      ...
      await context.log('TrialResult=FAILURE')
      state = State.FAILURE
      print(state)
      failure_sound.play()
      await context.log(f"TRIAL_NUM={trial_num}, \
                  STIM_TRIAL_SUCCESS_COUNT={trial_stim_success_count}, STIM_TRIAL_NUM={trial_stim_count}, STIM_SUCCESS_RATE={stim_trial_success_rate:.2f}%, \
